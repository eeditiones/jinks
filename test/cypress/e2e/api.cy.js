describe("API", () => {
    ...
    describe("default", () => {
        ...
        it.skip("POST /api/generator requires authentication (no dry run)", () => {
            ...
            cy.request({
                ...
                body: {
                    config: { id: "test-no-auth", label: "No Auth", type: "app" },
                    resolve: [],
                },
                failOnStatusCode: false,
            }).then((res) => {
        ...
        it("POST /api/generator can update app with CI profile without collection errors", () => {
            ...
            cy.request({
                ...
                cy.request({
                    ...
                    cy.wrap(responseStr).should(
                        "not.include",
                        "Collection .github/workflows not found",
                    );
                    ...
                    if (secondRes.body.messages) {
                        ...
                        const errorMessages = messages.filter(
                            (msg) =>
                                ...
                                typeof msg === "object" &&
                                (msg.type === "error" ||
                                    msg.message?.includes("Collection") ||
                                    msg.message?.includes("not found")),
                        );
                        cy.wrap(errorMessages.length).should(
                            "eq",
                            0,
    ...
    describe("view", () => {
        ...
        it("GET /{file}.md returns 404 for non-existent doc", () => {
            cy.request({ url: "/profiles/docs/does-not-exist.md", failOnStatusCode: false }).then(
                (res) => {
    ...
    describe("deploy", () => {
        it("POST /api/generator/{profile}/deploy returns stubbed success (no side effects)", () => {
            ...
            cy.wait("@deploy")
                .its("request.url")
                .should("include", `/api/generator/${profile}/deploy`);
