...
describe('Dynamic JSON Schema Validation', () => {
  ...
  it('validates jinks api.json against the JSON Schema https://spec.openapis.org/oas/3.0/schema/2021-09-28', () => {
  ...
  it.skip('validates all api.json files against OpenAPI 3.0 schema', () => {
    cy.findFiles('**/api.json').then(apiFiles => {
      cy.readFile(openApiSchema).then(schema => {
        ajv4.addSchema(schema, 'https://spec.openapis.org/oas/3.0/schema/2021-09-28')
