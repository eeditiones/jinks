describe("index page", () => {
    ...
    describe("nav bar", () => {
        it("should be visible", () => {
            cy.get("nav").should("be.visible").and("have.length.gte", 2);
        });
        ...
        it("mode button should toggle between light and dark", () => {
            cy.get("body")
                ...
                .then((initialTheme) => {
                    cy.get("#colorMode").should("be.visible").click();
                    cy.get("body")
                        .invoke("attr", "data-theme")
        ...
        it("menus should allow click navigation", () => {
            cy.get("nav > ul").should("have.length.gte", 3);
            cy.get(".dropdown > summary").contains("Tools").click();
            cy.get(".dropdown > ul").contains("Templating Tester").click();
            cy.url().should("include", "/templating.html");
            cy.get(".dropdown > summary").click();
    ...
    describe("content area", () => {
        it("should have the config tab selected by default", () => {
            ...
            cy.get('section[data-tab="config"]').should("be.visible");
        });

        it("should have abbrev filled in", () => {
            cy.get('[name="abbrev"]').type("e2edry{enter}");
        });
        ...
        it("should have the profiles tab", () => {
            ...
            cy.get("#profiles").find('[name="base"]').should("exist");
            cy.get("#profiles").find('[name="feature"]').should("exist");
        });
        ...
        it("should have the theming tab", () => {
            cy.get('nav.tabs > ul > li > a[href="#theming"]')
                .should("not.have.class", "active")
                .click();
            cy.get('section[data-tab="theming"]').should("be.visible");
        });

        it("theming tab should have a theme selected by default", () => {
            cy.get('nav.tabs > ul > li > a[href="#theming"]').click();
        ...
        it("the custom ODD picker should modify config", () => {
            cy.get('[name="custom-odd"]').type("e2etest.odd");
            cy.get("#add-odd").click();
            cy.get("#appConfig").contains('"odd": "e2etest.odd"');
        });
    ...
    describe("deployments [side-effects]", () => {
        ...
        it.skip("should simulate creating a custom app with selected features (dry mode)", () => {
            ...
            cy.get('[name="abbrev"]').type("e2edry{enter}");
            cy.get('[name="label"]').type(" application");
            cy.get('[name="id"]');
            cy.contains("https://e-editiones.org/apps/e2edry");
            cy.get('[type="checkbox"]').check("docker");
            cy.get("#appConfig").contains("docker");
            cy.get("#dry-run").click();
            cy.wait("@dryTest");
            cy.get(".error").contains("not.be.visible");
        });
        ...
        it("should force create a custom app with selected features [side-effects]", () => {
            ...
            cy.get('[name="abbrev"]').type("e2etest{enter}");
            cy.get('[name="label"]').type(" application");
            cy.get('[name="id"]');
            ...
            cy.get('[name="overwrite"]').select("all");
