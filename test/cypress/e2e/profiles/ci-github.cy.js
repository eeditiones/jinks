...
describe("GitHub Actions CI Templates", () => {
    ...
    describe("GitHub Actions workflow template", () => {
        ...
        it("POST /api/templates expands GitHub template without externalXar (no blank lines)", () => {
            cy.request({
                ...
                const dockerBuildkitIndex = lines.findIndex((line) =>
                    line.includes("DOCKER_BUILDKIT: 1"),
                );
                ...
                const runIndex = lines.findIndex(
                    (line, idx) =>
                        idx > dockerBuildkitIndex &&
                        idx < dockerBuildkitIndex + 10 &&
                        line.includes("run: |"),
                );
                cy.wrap(runIndex).should(
                    "be.greaterThan",
                    -1,
                    "run: | should appear after DOCKER_BUILDKIT",
                );
                ...
                if (runIndex >= 0) {
                    ...
                    if (loadIndex >= 0 && tagIndex >= 0 && tagIndex > loadIndex) {
                        ...
                        cy.wrap(blankLines.length).should(
                            "eq",
                            0,
                            `Found ${blankLines.length} blank line(s) between --load and -t in docker buildx build command`,
                        );
                    }
                }
                ...
                cy.wrap(validation.valid).should(
                    "be.true",
                    `YAML validation failed: ${validation.error || "unknown error"}`,
                );
                ...
                cy.wrap(result.split("\n").some((line) => errorPattern.test(line.trim()))).should(
                    "be.false",
                ...
                const debugLines = lines.slice(
                    Math.max(0, dockerBuildkitIndex - 2),
                    Math.min(lines.length, dockerBuildkitIndex + 10),
                );
        ...
        it("POST /api/templates expands GitHub template with GITHUB_TOKEN", () => {
            cy.request({
                ...
                cy.writeFile(
                    "test/cypress/downloads/github-with-github-token.yml",
                    res.body.result,
                );
                ...
                const errorPattern =
                    /^(Error|ERROR|error):\s|^Error while|^failed\s|^Failed\s|^FAILED\s/;
                cy.wrap(
                    res.body.result.split("\n").some((line) => {
                        const trimmed = line.trim();
                        return (
                            errorPattern.test(trimmed) &&
                            !trimmed.includes("allow_failure") &&
                            !trimmed.includes("CI_JOB_STATUS")
                        );
                    }),
        ...
        it("POST /api/templates expands GitHub template with custom token", () => {
            cy.request({
                ...
                // Save output file for debugging even if test fails
                cy.writeFile(
                    "test/cypress/downloads/github-with-custom-token.yml",
                    res.body.result,
                );
            ...
            scenarios.forEach((scenario) => {
                cy.request({
                    ...
                    cy.validateJsonSchema(
                        ...
                        schema.schema,
                        workflowJson,
                        `GitHub Actions workflow (${scenario.name})`,
                    );
                });
            });
        });
    ...
    describe("TEI Publisher Docker Publish workflow template", () => {
        ...
        before(() => {
            cy.readFile("profiles/ci/.github/workflows/tp-docker-publish.tpl.yml").then(
                (template) => {
