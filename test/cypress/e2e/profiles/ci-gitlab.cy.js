        ...
        cy.request({
            ...
            if (!schemaData || typeof schemaData !== "object" || Array.isArray(schemaData)) {
                ...
                cy.log(
                    ...
                    JSON.stringify(schemaData).substring(0, 500),
                );
    ...
    describe("GitLab CI template", () => {
        ...
        it("POST /api/templates expands GitLab template without externalXar (no blank lines)", () => {
            cy.request({
                ...
                const dockerBuildkitIndex = lines.findIndex((line) =>
                    line.includes("export DOCKER_BUILDKIT=1"),
                );
                ...
                const nextLine =
                    dockerBuildkitIndex + 1 < lines.length ? lines[dockerBuildkitIndex + 1] : "";
                cy.wrap(
                    dockerBuildkitLine.includes("docker buildx build") ||
                        nextLine.includes("docker buildx build"),
                ).should(
                    "be.true",
                    "docker buildx build should appear on same line or next line after export DOCKER_BUILDKIT=1",
                );
                ...
                cy.wrap(result).should(
                    "not.match",
                    /\[% .* %\]/,
                    "Template blocks should be processed, not left in output",
                );
        ...
        it("POST /api/templates expands GitLab template with CI_JOB_TOKEN", () => {
            cy.request({
                ...
                cy.writeFile(
                    "test/cypress/downloads/gitlab-with-ci-job-token.yml",
                    res.body.result,
                );
        ...
        it("POST /api/templates expands GitLab template with custom token", () => {
            cy.request({
                ...
                // Save output file for debugging even if test fails
                cy.writeFile(
                    "test/cypress/downloads/gitlab-with-custom-token.yml",
                    res.body.result,
                );
            ...
            scenarios.forEach((scenario) => {
                cy.request({
                    ...
                    cy.validateJsonSchema(
                        ...
                        schema.schema,
                        ciJson,
                        `GitLab CI workflow (${scenario.name})`,
                    );
                });
            });
        });
