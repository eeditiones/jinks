...
declare function nav:sort ($sortBy as xs:string, $items as element()*) {
    switch ($sortBy)
        case "date" return
            try { sort($items, (), ft:field(?, "date", "xs:date")) } catch * {
...
declare function nav:get-content ($config as map(*), $div as element()) {
    typeswitch ($div)
        ...
        case element(tei:pb) return
            ...
            let $chunk := nav:milestone-chunk(
                $div,
                $nextPage,
                if ($nextPage) then
                    ($div/ancestor::* intersect $nextPage/ancestor::*)[last()]
                else (: if there's only one pb in the document, it's whole
                      text part should be returned :) if (
                    count($div/ancestor::tei:text//tei:pb) = 1
                ) then (
...
declare function nav:get-subsections ($config as map(*), $root as node()) {
    ...
    let $subsections := if (count($headed)) then
        $headed
    else
        $root//tei:div except
            $root//tei:div//tei:div
...
declare function nav:get-section-heading ($config as map(*), $section as node()) {
    ...
    if (count($section/tei:head)) then
        $section/tei:head
    else
        $section
};
...
declare function nav:is-filler ($config as map(*), $div) {
    ...
    return if ($parent and nav:filler($config, $parent)/descendant-or-self::tei:div[. is $div]) then
        $parent
    else (
    )
};
...
declare function nav:filler ($config as map(*), $div) {
    ...
    return if (
        $config?fill > 0 and
            $child and
            count(($child)/preceding-sibling::*/descendant-or-self::*) < $config?fill
    ) then
        $child
    else (
    )
...
declare function nav:fill ($config as map(*), $div) {
    if (
        $div/tei:div and $config?fill > 0 and count($div/ancestor-or-self::tei:div) < $config?depth
    ) then
        let $filler := nav:filler($config, $div)
        return if ($filler) then
...
declare function nav:previous-page ($config as map(*), $div) {
    ...
    let $previous := if ($preceding << $parent) then
        $parent
    else
        $preceding
    return if ($previous) then
        (:
...
declare function nav:get-next ($config as map(*), $div as element(), $view as xs:string) {
    ...
    return if (
        empty($config?context) or
            $config?context instance of document-node() or
            $next/ancestor::*[. is $config?context]
    ) then
        $next
    else (
    )
...
declare function nav:get-previous ($config as map(*), $div as element(), $view as xs:string) {
    ...
    return if (
        $config?context instance of document-node() or $previous/ancestor::*[. is $config?context]
    ) then
        $previous
    else (
    )
...
declare function nav:milestone-chunk (
    $ms1 as element(),
    $ms2 as element()?,
    $node as node()*
) as node()* {
    let $descendantCheck := if (
        $ms1 instance of element(tei:pb) and (empty($ms2) or $ms2 instance of element(tei:pb))
    ) then
        function ($node, $ms1, $ms2) { $node/descendant::tei:pb intersect ($ms1, $ms2) }
    else
        function ($node, $ms1, $ms2) {
            some $n in $node/descendant::* satisfies ($n is $ms1 or $n is $ms2)
        }
    return nav:milestone-chunk($ms1, $ms2, $node, $descendantCheck)
};
