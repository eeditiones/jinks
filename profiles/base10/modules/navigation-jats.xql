    ...
    typeswitch ($div)
        case element(sec) return
            if ($div/sec and count($div/ancestor::sec) < $config?depth - 1) then
                if (
                    $config?fill > 0 and count(($div/sec[1])/preceding-sibling::*//*) < $config?fill
                ) then
                    let $child := $div/sec[1]
                    return element {node-name($div)} {
...
declare function nav:get-previous-div ($config as map(*), $div as element()) {
    ...
    return nav:get-previous-recursive(
        $config,
        if ($parent and (empty($prevDiv) or $div/.. >> $prevDiv)) then
            $div/..
        else
            $prevDiv
    )
...
declare %private function nav:get-previous-recursive ($config as map(*), $div as element(sec)?) {
    if (empty($div)) then (
        ...
        empty($div/preceding-sibling::sec) (: first div in section :) and
            $config?fill > 0 and
            count($div/preceding-sibling::*//*) <
                $config?fill (: less than 5 elements before div :) and
            $div/.. instance of element(sec) (: parent is a div :)
    ) then
        nav:get-previous-recursive(
            $config,
            $div/ancestor::sec[count(ancestor::sec) < $config?depth][1]
        )
    else
        $div
};
