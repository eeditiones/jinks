...
declare function nav:get-root ($root as xs:string?, $options as map(*)?) {
    $config:data-root!collection(. || "/" || $root)//dbk:article[ft:query(., "db-file:*", $options)]
};
...
declare function nav:filler ($config as map(*), $div) {
    if (
        $config?fill > 0 and
            $div/dbk:section and
            count(($div/dbk:section[1])/preceding-sibling::*/descendant-or-self::*) < $config?fill
    ) then
        $div/dbk:section[1]
    else (
    )
...
declare function nav:fill ($config as map(*), $div) {
    if (
        $div/dbk:section and
            $config?fill > 0 and
            count($div/ancestor-or-self::dbk:section) < $config?depth
    ) then
...
declare function nav:is-filler ($config as map(*), $div) {
    let $parent := $div/ancestor::dbk:section[count(ancestor-or-self::dbk:section) <=
        $config?depth][1]
    return if (
        $parent and nav:filler($config, $parent)/descendant-or-self::dbk:section[. is $div]
    ) then
        $parent
    else (
    )
...
declare function nav:previous-page ($config as map(*), $div) {
    let $preceding := $div/preceding::dbk:section[count(ancestor-or-self::dbk:section) <=
        $config?depth][1]
    ...
    return if ($previous) then
        (:
         ...
         :)
        let $nearest := filter(
            $previous/ancestor-or-self::dbk:section[count(ancestor-or-self::dbk:section) <=
                $config?depth],
            function ($ancestor) {
                exists(
                    nav:filler($config, $ancestor)/descendant-or-self::dbk:section[. is $previous]
                )
            }
        )[1]
