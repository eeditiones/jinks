...
declare function epub:generate-epub ($config as map(*), $doc, $css, $filename) {
    let $docConfig := map:merge(
        (tpu:parse-pi(root($doc), "div"), map {"view": "div"}),
        map {"duplicates": "use-last"}
    )
    let $config := map:merge(
        ($config, map {"docConfig": $docConfig}, map {"type": $docConfig?type}),
        map {"duplicates": "use-last"}
    )
...
declare function epub:mimetype-entry () {
    <entry method="store" name="mimetype" type="text">application/epub+zip</entry>
};

declare function epub:fonts-entry ($config as map(*)) {
    for $font in $config?fonts?*
    let $name := replace($font, "^.*/([^/]+)$", "$1")
    return <entry name="OEBPS/Fonts/{ $name }" type="binary">{ util:binary-doc($font) }</entry>
};
...
declare function epub:container-entry () {
    ...
    return <entry name="META-INF/container.xml" type="xml">{ $container }</entry>
};
...
declare function epub:content-opf-entry ($config as map(*), $text, $xhtml as element()*) {
    ...
    let $refs :=
        for $entry in $xhtml
        let $id := replace($entry/@name, "^OEBPS/(.*)\.xhtml$", "$1")
        return <itemref xmlns="http://www.idpf.org/2007/opf" idref="{ $id }" />
    let $content-opf := <package
        xmlns="http://www.idpf.org/2007/opf"
    ...
    >
        <metadata xmlns:dc="http://purl.org/dc/elements/1.1/">
            ...
            {
                if ($config?coverImage) then
                    <meta content="{ $config?coverImage }" name="cover" />
                else (
                )
            }
        </metadata>
        <manifest>
            ...
            <item href="stylesheet.css" id="css" media-type="text/css" />
            {
                ...
                let $media-type := switch ($suffix)
                    ...
                    default return
                        "image/" || $suffix
                return <item href="{ $img }" id="{ $img }" media-type="{ $media-type }">
                    {
                        if ($config?coverImage and $config?coverImage = $img) then
    ...
    return <entry name="OEBPS/content.opf" type="xml">{ $content-opf }</entry>
};

declare function epub:images-entry ($config, $doc, $entries as element()*) {
    let $root := if ($config?imagesCollection) then
        $config?imagesCollection
    else
        util:collection-name($doc)
    for $relPath in (distinct-values($entries//*:img/@src), $config?coverImage)
    let $path := if ($config:epub-images-path) then
...
declare function epub:title-xhtml-entry ($language, $doc, $config) {
    ...
    return <entry name="OEBPS/title.xhtml" type="xml">{ $title-xhtml }</entry>
};
...
declare function epub:body-xhtml ($node, $config) {
    ...
    let $body-xhtml := epub:assemble-xhtml(
        string-join($title),
        $config?metadata?language,
        epub:fix-namespaces($body)
    )
    return (
        <entry name="{ concat("OEBPS/", epub:generate-id($node), ".xhtml") }" type="xml">
            { $body-xhtml }
        </entry>,
        if ($next) then
...
declare function epub:nav-entry ($config, $text) {
    ...
    return <entry name="OEBPS/nav.xhtml" type="xml">{ $toc }</entry>
};
    ...
    <ol xmlns="http://www.w3.org/1999/xhtml">
        {
            ...
            let $id := if ($div/@xml:id) then
                $div/@xml:id
            else
                epub:generate-id($div)
            return <li>
...
declare function epub:toc-ncx-entry ($config, $text) {
    let $toc-ncx := <ncx xmlns="http://www.daisy.org/z3986/2005/ncx/" version="2005-1">
        <head>
            <meta content="{ $config?metadata?urn }" name="dtb:uid" />
            <meta content="2" name="dtb:depth" />
            <meta content="0" name="dtb:totalPageCount" />
            <meta content="0" name="dtb:maxPageNumber" />
        </head>
        <docTitle><text>{ $config?metadata?title }</text></docTitle>
        <navMap>
            {
                if ($config?skipTitle = true()) then (
                ) else
    ...
    return <entry name="OEBPS/toc.ncx" type="xml">{ $toc-ncx }</entry>
};
...
declare function epub:toc-ncx-div ($config, $root as element(), $start as xs:int) {
    ...
    return <navPoint
        ...
        id="navpoint-{ $id }"
        playOrder="{ counter:next-value("teipublisher-epub-toc") + $start }"
    >
        <navLabel><text>{ $html }</text></navLabel>
...
declare function epub:strip-footnotes ($nodes as node()*) {
    ...
    return typeswitch ($node)
        case document-node() return
            document { epub:strip-footnotes($node/node()) }
        case element(xhtml:aside) return
            if ($node[@ep:type = "footnote"]) then (
            ) else
                $node
        case element() return
            element {node-name($node)} { $node/@*, epub:strip-footnotes($node/node()) }
    ...
    return typeswitch ($node)
        case element() return
            element {QName("http://www.w3.org/1999/xhtml", local-name($node))} {
                ...
                return epub:fix-namespaces($child)
            }
