...
declare function cutil:check-last-modified (
    $request as map(*),
    $nodes as node()*,
    ...
    if ($config:enable-proxy-caching) then (
        ...
        let $ifModifiedSince := if (map:contains($request?parameters, "If-Modified-Since")) then
            $request?parameters?If-Modified-Since => parse-ietf-date()
        else (
        )
        let $lastModified :=
            for $node in $nodes
            let $modified := xmldb:last-modified(
                util:collection-name($node),
                util:document-name($node)
            )
            order by $modified descending
            return $modified
        let $shouldReturn304 := if (exists($ifModifiedSince)) then
            $ifModifiedSince ge
                $lastModified
                    (:
        ...
        return (
            if (string-length($lastModified) > 0) then (
