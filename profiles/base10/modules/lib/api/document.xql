...
declare function dapi:metadata ($request as map(*)) {
    ...
    return if (exists($xml)) then
        ...
        return map {
            ...
            "lastModified":
                xmldb:last-modified(util:collection-name($xml), util:document-name($xml))
        }
    else
    ...
    if (exists($components)) then
        ...
        return (
            if (not(xmldb:collection-available($collection || "/" || $components[1]))) then
                ...
                return (
                    sm:chown(xs:anyURI($created), $dapi:repoxml//repo:permissions/@user),
                    sm:chgrp(xs:anyURI($created), $dapi:repoxml//repo:permissions/@group),
                    sm:chmod(
                        xs:anyURI($created),
                        replace(
    ...
    return if ($doc) then
        ...
        return if (exists($xml)) then
            ...
            let $styles := (
                ...
                <link
                    href="transform/{ replace($config?odd, "^.*?/?([^/]+)\.odd$", "$1") }.css"
                    rel="stylesheet"
                    type="text/css" />
            )
            return dapi:postprocess(
                ($out[2], $out[1])[1],
                $styles,
                $addScripts,
                $request?parameters?base,
                $request?parameters?wc
            )
        else
            error($errors:NOT_FOUND, "Document " || $doc || " not found")
    else
...
declare function dapi:postprocess (
    $nodes as node()*,
    $styles as element()*,
    $scripts as element()*,
    $base as xs:string?,
    ...
    return typeswitch ($node)
        ...
        case element(body) return
            let $content := (
                ...
                let $footnotes :=
                    ...
                    return element {node-name($fn)} {
                        $fn/@*, dapi:postprocess($fn/node(), $styles, $scripts, $base, $components)
                    }
                return nav:output-footnotes($footnotes)
            )
        ...
        case element() return
            if ($node/@class = "footnote") then (
                ...
                element {node-name($node)} {
                    $node/@*, dapi:postprocess($node/node(), $styles, $scripts, $base, $components)
                }
...
declare %private function dapi:webcomponents ($components as xs:string?) {
    if ($components) then (
        ...
        <script
            defer="defer"
            src="https://cdn.jsdelivr.net/npm/web-components-loader/lib/index.min.js" />,
        switch ($config:webcomponents)
            case "dev" return
                <script
                    src="{ $config:webcomponents-cdn }/src/pb-components-bundle.js"
                    type="module" />
            case "local" return
                <script src="resources/scripts/pb-components-bundle.js" type="module" />
            default return
                <script
                    src="{ $config:webcomponents-cdn }@{
    ...
    return (
        ...
        if ($id) then
            ...
            return if (exists($xml)) then
                ...
                let $file := replace($id, "^.*?([^/]+)$", "$1") ||
                    format-dateTime(current-dateTime(), "-[Y0000][M00][D00]-[H00][m00]")
                return if ($source) then
                    router:response(200, "application/x-latex", $tex)
                else
                    let $serialized := file:serialize-binary(
                        util:string-to-binary($tex),
                        $config:tex-temp-dir || "/" || $file || ".tex"
                    )
                    let $options := <option>
                        <workingDir>{ $config:tex-temp-dir }</workingDir>
                    </option>
                    let $outputPath := $config:tex-temp-dir || "/" || $file || ".pdf"
                    let $cleanup := if (file:exists($outputPath)) then
                        file:delete($outputPath)
                    else (
                    )
                    let $output0 := process:execute(($config:tex-command($file)), $options)
                    return if (not(file:exists($outputPath))) then
                        error(
                            $errors:BAD_REQUEST,
                            "LaTeX reported errors",
                            dapi:latex-error($output0)
                        )
                    else
                        let $output :=
                            for $i in 1 to 2
                            return process:execute(($config:tex-command($file)), $options)
                        return let $pdf := file:read-binary(
                                $config:tex-temp-dir || "/" || $file || ".pdf"
                            )
...
declare function dapi:get-cached ($id as xs:string, $doc as node()) {
    ...
    return if ($dapi:CACHE and util:binary-doc-available($path)) then
        ...
        return if ($modDatePDF >= $modDateSrc) then
...
declare function dapi:pdf ($request as map(*)) {
    ...
    return if ($doc) then
        let $cached := if ($useCache) then
            dapi:get-cached($name, $doc)
        else (
        )
        return (
            response:set-cookie("simple.token", $token),
...
declare function dapi:markdown ($request as map(*)) {
    ...
    return if (exists($doc)) then
        ...
        return router:response(200, "text/markdown; charset=utf-8", string-join($markdown, ""))
    else
        error($errors:NOT_FOUND, "Document " || $id || " not found")
};
...
declare function dapi:epub ($request as map(*)) {
    ...
    return if (exists($work)) then
        ...
        return (
            ...
            response:set-header(
                "Content-Disposition",
                concat("attachment; filename=", concat($id, ".epub"))
            ),
            response:stream-binary(
...
declare %private function dapi:work2epub (
    $request as map(*),
    $id as xs:string,
    $work as document-node(),
    $lang as xs:string?
) {
    ...
    let $cssDefault := util:binary-to-string(
        util:binary-doc($config:output-root || "/" || $oddName || ".css")
    )
    let $cssEpub := util:binary-to-string(
        util:binary-doc($config:app-root || "/resources/css/epub.css")
    )
    let $css := $cssDefault || "&#10;/* styles imported from epub.css */&#10;" || $cssEpub
    return epub:generate-epub($config, $work/*, $css, $id)
};
...
declare function dapi:get-fragment ($request as map(*), $docs as node()*, $path as xs:string) {
    ...
    let $xml := if (
        exists($request?parameters?id) and
            $request?parameters?id != "" and
            $request?parameters?view != "single"
    ) then
        for $document in $docs
        let $config := tpu:parse-pi(root($document), $view)
        let $context := dapi:apply-xpath($request, $document)
        let $data := if (count($request?parameters?id) = 1) then
            if ($view = "div") then
    ...
    return if ($xml?data) then
        let $userParams := map:merge(
            (
                request:get-parameter-names()[starts-with(., "user")]!map {
                    substring-after(., "user."): request:get-parameter(., ())
                },
                map {"webcomponents": 7}
            )
        )
        let $mapped := if ($request?parameters?map) then
            ...
            $xml?data
        let $data := if (
            empty($request?parameters?xpath) and
                request:get-parameter("user.highlight", ()) and
                exists(session:get-attribute($config:session-prefix || ".search"))
        ) then
            query:expand($xml?config, $mapped)[1]
        else
            $mapped
        let $content := if (not($view = "single")) then
            pages:get-content($xml?config, $data)
        else
            $data

        let $html := typeswitch ($mapped)
        ...
        return if ($request?parameters?format = "html") then
            router:response(200, "text/html", $transformed?content)
        else
            let $next := if ($view = "single") then (
            ) else
                $config:next-page($xml?config, $xml?data, $view)
            let $prev := if ($view = "single") then (
            ) else
                $config:previous-page($xml?config, $xml?data, $view)
            return router:response(
                200,
                "application/json",
                map {
...
declare %private function dapi:apply-xpath ($request as map(*), $data as node()) {
    if ($request?parameters?xpath) then
        ...
        let $xquery := "declare default element namespace '" ||
            $namespace ||
            "'; $data" ||
...
declare function dapi:table-of-contents ($request as map(*)) {
    ...
    return if ($documents) then (
        cutil:check-last-modified(
            $request,
            $documents,
            function ($request as map(*), $documents as node()*) {
                let $xml := pages:load-xml($documents, $request?parameters?view, (), $doc)
                return if (exists($xml)) then
                    let $mapped := if (exists($request?parameters?map)) then
                        let $mapFun := function-lookup(
                            xs:QName("mapping:" || $request?parameters?map),
                            2
                        )
                        return if (exists($mapFun)) then
                            $mapFun($xml?data, $request?parameters)
                        else
                            $xml?data
                    else
                        $xml?data
                    return dapi:toc-div(root($mapped), $xml, $request?parameters?target, $collapse)
                else
                    error($errors:NOT_FOUND, "Document " || $doc || " not found")
            }
        )
...
declare %private function dapi:toc-div (
    $node,
    $model as map(*),
    $target as xs:string?,
    ...
    return <ul>
        {
            ...
            let $html := if ($headings) then
                $pm-config:web-transform(
                    $headings,
                    map {"mode": "toc", "root": $div},
                    $model?config?odd
                )
            else (
            )
            ...
            let $parent := if ($view = "page") then (
            ) else
                nav:is-filler($model?config, $div)
            let $hasDivs := exists(nav:get-subsections($model?config, $div))
            let $nodeId := if ($parent) then
                util:node-id($parent)
            else
                util:node-id($root)
            let $xmlId := if ($parent) then
                $parent/@xml:id
            else
                $root/@xml:id
            let $hash := if (
                $view != "page" and not(nav:get-section-for-node($model?config, $div) is $root)
            ) then
                if ($root/@xml:id) then
...
declare function dapi:preview ($request as map(*)) {
    ...
    let $html := $pm-config:web-transform(
        $request?body,
        map {"root": $request?body, "webcomponents": 7},
        $config?odd
    )
    let $styles := <link
        href="transform/{ replace($config?odd, "^.*?/?([^/]+)\.odd$", "$1") }.css"
        rel="stylesheet"
        type="text/css" />
    return dapi:postprocess($html, $styles, (), $request?parameters?base, $request?parameters?wc)
};

declare function dapi:convert-docx ($request as map(*)) {
    let $transform := $pm-config:tei-transform(?, ?, $request?parameters?odd)
    return docx:process-pkg($request?body, $transform)
};
