...
declare function capi:list ($request as map(*)) {
    ...
    let $path := if ($request?parameters?path) then
        xmldb:decode($request?parameters?path)
    else (
    )
    ...
    let $start := if ($request?parameters?start > $total) then
    ...
    return (
        ...
        if ($request?parameters?format = "html") then
            ...
            let $templateAvail := doc-available($templatePath) or
                util:binary-doc-available($templatePath)
            let $path := if ($templateAvail and $worksAll?mode = "browse") then
                $templatePath
            else
                $config:app-root || "/templates/documents.html"
            let $template := if (doc-available($path)) then
                doc($path) => serialize()
            ...
            let $model := map:merge(
                (
                    ...
                    map {"documents": $works, "language": $request?parameters?language}
                )
            )
            return tmpl:process(
                $template,
                    ...
                    "modules":
                        map {
                            "http://www.tei-c.org/tei-simple/config":
                                map {"prefix": "config", "at": "modules/config.xqm"}
                        }
                }
            )
        else
...
declare %private function capi:list-works ($root as xs:string?, $cached, $params as map(*)) {
    (: session:clear(), :)
    let $sort := head(($params?sort, $config:sort-default))
    let $filter := $params?field
    let $query := $params?query
    let $filtered := if (exists($cached)) then
        $cached
    else
    ...
    return array
        {
            ...
            let $header := $pm-config:web-transform(
                $teiHeader,
                map {
                    "display": "browse",
                    "doc": $relPath,
            ...
            return map {
                ...
                "view": $config?view,
                "lastModified":
                    xmldb:last-modified(util:collection-name($doc), util:document-name($doc))
            }
        }
};
...
declare %private function capi:params2map ($root as xs:string?) {
    map:merge(
        (
            for $param in
                request:get-parameter-names()[not(. = ("start", "per-page", "page", "path"))]
            return map:entry($param, request:get-parameter($param, ())),
...
declare %private function capi:use-cache ($params as map(*), $cached) {
    ...
    return if (
        exists($cached) and
            exists($cachedParams) and
            deep-equal($params, $cachedParams) and
            exists($timestamp)
    ) then
...
declare function capi:upload ($request as map(*)) {
    ...
    return array { capi:upload($request?parameters?collection, $name, $data) }
};
...
declare %private function capi:upload ($root, $paths, $payloads) {
    for-each-pair(
        ...
        $payloads,
        function ($path, $data) {
            let $path := if (ends-with($path, ".odd")) then
                ...
                return if (xmldb:collection-available($collectionPath)) then
                    if (ends-with($path, ".docx")) then
                        let $mediaPath := $config:data-default ||
                            "/" ||
                            $root ||
                            "/" ||
                            xmldb:encode($path) ||
                        ...
                        let $tei := docx:process(
                            $stored,
                            $config:data-default,
                            $pm-config:tei-transform(?, ?, "docx.odd"),
                            $mediaPath
                        )
                        let $teiDoc := document {
                            processing-instruction teipublisher { $config:default-docx-pi }, $tei
                        }
                        return xmldb:store($collectionPath, xmldb:encode($path) || ".xml", $teiDoc)
                    else
