...
declare variable $oapi:EXIDE := let $path := collection(repo:get-root())//expath:package[@name =
...
declare function oapi:load-source ($href as xs:string, $line as xs:int?) {
    ...
    return <a
        class="eXide-open"
        data-exide-line="{ $line }"
        data-exide-open="{ $href }"
        href="{ $link }"
        target="eXide"
    >{ $href }</a>
};
...
declare function oapi:get-line ($src, $line as xs:int?) {
    if ($line) then
        ...
        return subsequence($lines, $line - 1, 3)!replace(., "^\s*(.*?)", "$1&#10;")
    else (
    )
};
...
declare function oapi:recompile ($request as map(*)) {
    ...
    let $result :=
        ...
        return try {
            ...
            return if ($output?error) then
                <div class="list-group-item-danger">
                    <h5 class="list-group-item-heading">{ $file }: ERROR</h5>
                    <p class="list-group-item-text">{ $output?error/error/string() }</p>
                    <h5 class="list-group-item-heading">Compilation error on line {
                            $output?error/error/@line/string()
                        }:</h5>
                    <pre class="list-group-item-text">
                        { oapi:get-line($output?code, $output?error/error/@line) }
                    </pre>
                ...
                return if ($compiled/error) then
                    <div class="list-group-item-danger">
                        <h5 class="list-group-item-heading">
                            { oapi:load-source($file, $compiled/error/@line) }:</h5>
                        <p class="list-group-item-text">{ $compiled/error/string() }</p>
                        <pre class="list-group-item-text">
                            { oapi:get-line($src, $compiled/error/@line) }
                        </pre>
                    </div>
                else
...
declare %private function oapi:parse-template ($template as xs:string, $title as xs:string?) {
    let $context := map {"label": $title}
    return tmpl:process(
        $template,
        $context,
        map {"plainText": false(), "ignoreImports": true(), "ignoreUse": true()}
    )
};
...
declare function oapi:create-odd ($request as map(*)) {
    ...
    let $stored := xmldb:store(
        $config:odd-root,
        $request?parameters?odd || ".odd",
        $parsed,
        "text/xml"
    )
    return (
        oapi:compile($request?parameters?odd),
        router:response(201, "application/json", map {"path": $stored})
    )
};
...
declare function oapi:save-odd ($request as map(*)) {
    ...
    return if ($canWrite) then
        ...
        return router:response(
            201,
            "application/json",
            map {"path": $stored, "report": $report, "source": $updated}
        )
    else
        router:response(
            401,
    ...
    array
        {
            ...
            return map {"name": $param/@name/string(), "value": $param/@value/string()},
...
declare function oapi:find-spec ($oddPath as xs:string, $root as xs:string, $ident as xs:string) {
    ...
    return if ($spec) then
        map {"status": "found", "odd": $oddPath, "models": oapi:models($spec)}
    else
...
declare function oapi:get-odd ($request as map(*)) {
    let $root := head(($request?parameters?root, $config:odd-root))
    return if (exists($request?parameters?ident)) then
        router:response(
            200,
            "application/json",
        ...
        return if (doc-available($path)) then
            let $odd := doc($path)
            return if ("application/json" = router:accepted-content-types()) then
...
declare function oapi:lint ($request as map(*)) {
    ...
    let $query :=
    ``[xquery version "3.1";import module namespace global="http://www.tei-c.org/tei-simple/config" at "../modules/config.xqm";declare variable $parameters := map {};declare variable $mode := '';declare variable $node := ();declare variable $get := (); () ! (
`{$code}`
)]``
    let $outputRoot := head(($request?parameters?output-root, $config:output-root))
    let $r := util:compile-query($query, $outputRoot)
    return if ($r/@result = "fail") then
        let $error := $r/*:error
        let $msg := $error/string()
        let $analyzed := analyze-string($msg, ".*line:?\s(\d+).*?column\s(\d+)")
        let $analyzed := if ($analyzed//fn:group) then
            $analyzed
        else
            analyze-string($msg, "line\s(\d+):(\d+)")
        let $parsedLine := $analyzed//fn:group[1]
        let $parsedColumn := $analyzed//fn:group[2]
        let $line := if ($parsedLine) then
            number($parsedLine)
        else
            number($error/@line)
        let $column := if ($parsedColumn) then
            number($parsedColumn)
        else
            number($error/@column)
        let $lineCount := count(analyze-string($code, "\n")//fn:match)
        return map {
            "status": "fail",
            "line":
                if ($line < 2 or $line - 1 > $lineCount) then
                    1
                else
                    $line - 1,
            "rline": $error,
...
declare function oapi:update (
    $nodes as node()*,
    $data as document-node(),
    $orig as document-node()
) {
    for $node in $nodes
    return typeswitch ($node)
        case document-node() return
            document { oapi:update($node/node(), $data, $orig) }
        ...
        case element(titleStmt) return
            element {node-name($node)} {
                $node/@*, $data/schemaSpec/title[text()], $node/* except $node/title
            }
        ...
        case element() return
            element {node-name($node)} { $node/@*, oapi:update($node/node(), $data, $orig) }
...
declare %private function oapi:normalize-ns ($nodes as node()*) {
    ...
    return typeswitch ($node)
        ...
        case element(pb:behaviour)|element(pb:param)|element(pb:set-param) return
            element {node-name($node)} { $node/@*, oapi:normalize-ns($node/node()) }
        case element(pb:template) return
            <pb:template xml:space="preserve">{ $node/node() }</pb:template>
        case element() return
            element {QName(namespace-uri($node), local-name($node))} {
                $node/@*, oapi:normalize-ns($node/node())
            }
...
declare function oapi:add-tags-decl ($nodes as node()*) {
    ...
    return typeswitch ($node)
        case document-node() return
            document { oapi:add-tags-decl($node/node()) }
        case element(TEI) return
            element {node-name($node)} {
                for $prefix in in-scope-prefixes($node)[. != ""][. != "xml"][. != "xmlns"]
                let $namespace := namespace-uri-for-prefix($prefix, $node)
        ...
        case element(teiHeader) return
            element {node-name($node)} {
                ...
                if ($node/encodingDesc) then
                    ...
                    <encodingDesc><tagsDecl /></encodingDesc>,
