...
declare function nlp:status ($request as map(*)) {
    ...
    } catch * { error($errors:NOT_FOUND, "Failed to connect to NER API endpoint") }
};
...
declare function nlp:entity-recognition ($request as map(*)) {
    ...
    let $docs := if (exists($doc)) then
        $doc
    else
        collection($config:data-root || "/" || $path)/tei:TEI/tei:text
    return nlp:entity-recognition($request, $docs)
};
...
declare %private function nlp:entity-recognition ($request as map(*), $docs as element()*) {
    map:merge(
        ...
        return if ($request?parameters?debug) then
            map {config:get-relpath($doc): map {"plain": $plain, "offsets": $offsets}}
        else
...
declare function nlp:strings ($request as map(*)) {
    map:merge(
        ...
        let $docs := if (doc-available($config:data-root || "/" || $path)) then
        ...
        return switch ($format)
            case "annotations" return
                if (array:size($matches) > 0) then
                    ...
                    let $offsets := nlp:mapping-table($pairs, 0, $request?parameters?debug)
                    return nlp:convert($matches, $offsets, $doc)
                else (
                )
            default return
                if (array:size($matches) > 0) then
                    map {config:get-relpath($doc): $matches}
                else (
                )
    )
};
...
declare function nlp:matches-to-annotations ($request as map(*)) {
    ...
    return if (array:size($matches) > 0 and $doc) then
        let $pairs := (
            nlp:extract-plain-offsets($doc, true(), true()),
            nlp:extract-plain-offsets($doc//tei:note, false(), true())
        )
...
declare %private function nlp:match-string (
    $string as xs:string+,
    $type as xs:string,
    $properties as map(*),
    $text as xs:string
...
declare function nlp:plain-text ($request as map(*)) {
    ...
    let $doc := config:get-document($path)/tei:TEI/tei:text
    let $text := (
        nlp:extract-plain-text($doc, true()), nlp:extract-plain-text($doc//tei:note, false())
    )
    let $plain := string-join($text)
    return $plain
};
...
declare function nlp:train ($request as map(*)) {
    ...
    let $input := if ($document) then
        $document//tei:body
    else
        collection($config:data-root || "/" || $path)//tei:body
    return if (exists($input)) then
        for $doc in $input
        return (
...
declare function nlp:log ($request as map(*)) {
    ...
    return try {
        ...
        return router:response($response[1]/@status, "text/text", $response[2])
    } catch * { error($errors:NOT_FOUND, "Failed to connect to NER API endpoint") }
};

declare %private function nlp:training-data-from-blocks (
    $blocks as element()*,
    $processFn as xs:boolean?
) {
    for $block in $blocks
...
(:~
 ...
 :)
declare %private function nlp:collect-training-entities (
    $data as array(*)*,
    $offset as xs:int,
    $result as array(*)*
) {
    if (exists($data)) then
        ...
        return (
            if ($head?1 = $nlp-config:entities) then
                nlp:collect-training-entities(
                    tail($data),
                    $end,
                    ($result, [$offset, $end, $head?1])
                )
            else
...
(:~
 ...
 :)
declare function nlp:extract-plain-text (
    $nodes as node()*,
    $skipNotes as xs:boolean?,
    $regex as xs:string?,
    $type as xs:string?,
    $properties as map(*)?
) {
                    ...
                    $ancestor and
                        (
                            anno-config:entity-type($ancestor) != $type or
                                $properties($anno-config:reference-key) =
                                    anno-config:get-key($ancestor)
                        )
                ) then
                    string-join(
                        for $i in (1 to string-length($node))
                        return "&#x25A1;"
                    )
                else
                    $node/string()
...
declare %private function nlp:check-existing-annotations (
    $node as node(),
    $allowText as xs:boolean?
) {
    let $parent := $node/..
    return if (not($parent) or (not($allowText) and exists($parent/text()))) then (
...
(:~
 ...
 :)
declare function nlp:extract-plain-offsets (
    $nodes as node()*,
    $skipNotes as xs:boolean?,
    $trackOffsets as xs:boolean?
) {
    for $node at $pos in $nodes
    return typeswitch ($node)
        case document-node() return
        ...
        case element(tei:note) return
            if ($skipNotes) then (
                ...
                (: output empty line before footnote starts :)
                [(), " ", 0], nlp:extract-plain-offsets($node/node(), $skipNotes, $trackOffsets)
            )
        case element() return
            nlp:extract-plain-offsets($node/node(), $skipNotes, $trackOffsets)
        case text() return
            let $parent := $node/..
            let $text := if (matches($node, "^[\s\n]+$")) then
                " "
            else
                $node/string()
            return (: if there are preceding siblings, we need to calculate the absolute character offset within the parent node :) if (
                $pos > 1 and $trackOffsets
            ) then
...
declare %private function nlp:absolute-offset ($nodes as node()*, $start as xs:int) {
    if ($nodes) then
        ...
        return nlp:absolute-offset(tail($nodes), $offset)
    else
        $start
};
...
declare function nlp:mapping-table (
    $result as map(*)*,
    $pairs as array(*)*,
    $accum as xs:int,
    $debug as xs:boolean?
    ...
    if (empty($pairs)) then
        ...
        let $entry := if (exists($pair?1)) then
            map {
                "node":
                    if ($debug) then
                        util:node-id($pair?1)
        ...
        return nlp:mapping-table(($result, $entry), tail($pairs), $end, $debug)
};
...
declare function nlp:convert ($entities as array(*), $offsets as map(*)*, $doc as node()) {
    let $entries :=
        ...
        let $insertPoint := filter(
            $offsets,
            function ($offset as map(*)) {
        ...
        let $insertPoint := if ($ancestor) then
            map:merge(($insertPoint, map {"node": $ancestor}))
        else
            $insertPoint
        let $existingType := if ($insertPoint?node) then
            anno-config:entity-type($insertPoint?node)
        else (
        )
        return (: element is not marked as entity :) if (
            exists($insertPoint) and empty($existingType)
        ) then
            ...
            return map {
                ...
                "properties":
                    if (exists($entity?properties)) then (
                        ...
                        map {$anno-config:reference-key: ""}
            }
        (: element is marked as entity of different type or already has correct key set: ignore :)
        else if (
            exists($existingType) and
                (
                    $existingType != $entity?type or
                        (
                            exists($entity?properties) and
                                $entity?properties($anno-config:reference-key) =
                                    anno-config:get-key($insertPoint?node)
                        )
                )
        ) then (
        ) (: element is marked as entity of same type and has properties :) else if (
            exists($entity?properties)
        ) then
            map {
                "type": "modify",
    ...
    return map {config:get-relpath($doc): array { $entries }}
};
...
declare %private function nlp:check-ancestors ($node as element(), $entityType as xs:string) {
    let $parent := $node/..
    return if (not($parent) or exists($parent/text())) then (
...
declare function nlp:pattern-recognition ($request as map(*)) {
    map:merge(
        ...
        let $plain := string-join($pairs!.?2)
        return if ($request?parameters?debug) then
            map {"plain": $plain, "offsets": $offsets, "patterns": $patterns}
        else
    ...
    return [
        map {
            "lower":
                map {
                    "regex":
                        substring($forename, 1, 1) || "(?:\.|" || substring($forename, 2) || ")"
                },
            "op": "?"
        },
        map {"lower": map {"regex": lower-case($surname) || "\'?s?"}}
    ]
};

declare function nlp:entities-remote ($input as xs:string*, $model as xs:string) {
    let $request := <http:request method="POST" timeout="10">
...
declare function nlp:patterns-remote (
    $input as xs:string*,
    $patterns as array(*),
    $lang as xs:string
) {
    ...
    let $body := map {"lang": $lang, "text": string-join($input), "patterns": $patterns}
