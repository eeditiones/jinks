    ...
    return array
        {
            for $item in $items
            group by $item
            return map {"text": $item, "value": $item}
        }
};
        ...
        let $hitsAll :=
            ...
            for $hit in
                query:query-default(
                    ...
                    $request?parameters?doc,
                    ()
                )
            order by ft:score($hit) descending
            return $hit
        let $hitCount := count($hitsAll)
        let $hits := if ($hitCount > 1000) then
            subsequence($hitsAll, 1, 1000)
        else
            $hitsAll
...
declare %private function sapi:show-hits (
    $request as map(*),
    $hits as item()*,
    $docs as xs:string*
) {
    ...
    let $parent-id := if (exists($docs)) then
        replace($parent-id, "^.*?([^/]*)$", "$1")
    else
        $parent-id
        ...
        <div class="matches">
            {
                ...
                let $docLink := if ($config?view = "page") then
                    ...
                    return if ($pbBefore) then
                        ...
                        return if ($page) then
                            ...
                            util:node-id($div)
                else (: Check if the document has sections, otherwise don't pass root :) if (
                    nav:get-section-for-node($config, $div)
                ) then
                    util:node-id($div)
                else (
                )
                let $config := <config
                    ...
                    }action=search&amp;view={ $config?view }&amp;odd={ $config?odd }#{ $matchId }"
                    table="no"
                    width="60" />
    ...
    return <div>
        {
            ...
            return facets:display($config, $hits)
        }
    </div>
};
...
declare function sapi:list-facets ($request as map(*)) {
    ...
    let $facetConfig := filter(
        $facets-config:facets?*,
        function ($facetCfg) { $facetCfg?dimension = $type }
    )
    let $hits := session:get-attribute($config:session-prefix || ".hits")
    let $facets := ft:facets($hits, $type, ())
    let $matches :=
        for $key in
            if (exists($request?parameters?value)) then
                $request?parameters?value
            else
                map:keys($facets)
        let $label := facets:translate($facetConfig, $lang, $key)
        return map {"text": $label, "freq": $facets($key), "value": $key}

    let $filtered := filter(
        $matches,
        function ($item) { matches($item?text, "(?:^|\W)" || $request?parameters?query, "i") }
    )
