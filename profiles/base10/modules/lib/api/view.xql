...
declare %private function vapi:get-config ($doc as xs:string, $view as xs:string?) {
    ...
    return if (exists($document)) then
...
declare function vapi:load-config-json ($request as map(*)?) {
    let $context := parse-json(
        util:binary-to-string(util:binary-doc($config:app-root || "/context.json"))
    )
    return map:merge(
        (
            $context,
...
declare %private function vapi:merge-config ($jsonConfig as map(*), $docConfig as map(*)) {
    let $cleanedConfig := map:merge(
        (
            ...
            for $key in
                map:keys($docConfig)[not(
                    . = ("odd", "fill", "depth", "media", "view", "template", "output", "type")
                )]
            return map:entry($key, $docConfig($key))
        )
    )
    return tmpl:merge-deep(($jsonConfig, $cleanedConfig))
};
...
declare function vapi:view ($request as map(*)) {
    let $path := if ($request?parameters?suffix) then
    ...
    let $templateName := head(
        (vapi:get-template($config, $request?parameters?template), $config:default-template)
    )
    let $templatePaths := (
        $config:app-root || "/templates/pages/" || $templateName,
        $config:app-root || "/templates/" || $templateName
    )
    let $template := for-each(
        $templatePaths,
        function ($path) {
    ...
    return if (not($template)) then
        ...
        let $model := map:merge(
            (
                ...
                map {
                    ...
                    "media":
                        if (map:contains($config, "media")) then
                            $config?media
                        else (
                        )
                }
        ...
        return tmpl:process(
            ...
            map {
                ...
                "namespaces": map {"tei": "http://www.tei-c.org/ns/1.0"}
            }
        )
};
    ...
    let $model := map:merge(
        (
            ...
            if ($error?value?code) then
                map {"code": $error?value?code, "line": $error?value?line}
            else (
            )
        )
    )
    let $html := tmpl:process(
        $template,
        $model,
        map {
            "plainText": false(),
...
declare function vapi:html ($request as map(*), $extConfig as map(*)?) {
    let $path := $config:app-root ||
        "/templates/" ||
        xmldb:decode($request?parameters?file) ||
        ".html"
    let $template := if (doc-available($path)) then
        doc($path) => serialize()
    else if (util:binary-doc-available($path)) then
    ...
    let $config := map:merge(
        (vapi:load-config-json($request), map {"context-path": $config:context-path}, $extConfig)
    )
...
declare function vapi:text ($request as map(*)) {
    ...
    let $config := map:merge(
        (vapi:load-config-json($request), map {"context-path": $config:context-path})
    )
    return tmpl:process(
        $template,
        $config,
