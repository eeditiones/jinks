...
declare variable $action:repoxml := let $uri := doc($config:app-root || "/expath-pkg.xml")/*/@name
...
declare function action:fix-odds ($request as map(*)) {
    action:generate-pm-config(), action:generate-code()
};

declare %private function action:generate-pm-config () {
    let $pmuConfig := pmc:generate-pm-config(
    ...
    return map {"type": "action:fix-odds", "message": "recreated pm-config.xql"}
};
...
declare %private function action:generate-code () {
    ...
    return action:report-odd-status($report),
    ...
    return (
        ...
        return (sm:chown($path, $permissions/@user), sm:chgrp($path, $permissions/@group))
    )
};
