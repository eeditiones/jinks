...
declare function pages:load-xml (
    $data as node()*,
    $view as xs:string?,
    $root as xs:string?,
    $doc as xs:string
) {
    let $config := (: parse processing instructions and remember original context :) map:merge(
        (tpu:parse-pi(root($data[1]), $view), map {"context": $data})
    )
    return map {
        "config": $config,
        "data":
            switch ($config?view)
                case "div" return
                    if ($root) then
                        ...
                        return nav:get-section-for-node($config, $node)
                    else
...
declare function pages:process-content (
    $xml as node()*,
    $root as node()*,
    $config as map(*),
    $userParams as map(*)?,
    ...
    let $params := map:merge(
        (
            map {"root": $root, "view": $config?view, "context-path": $config:context-path},
            $userParams
        )
    )
    let $html := $pm-config:web-transform($xml, $params, $config?odd)
    let $class := if ($html//*[@class = ("margin-note")]) then
        "margin-right"
    else (
    )
    ...
    let $footnotes :=
        ...
        return element {node-name($fn)} { $fn/@*, pages:clean-footnotes($fn/node()) }
    let $content := (
        $body,
        if ($footnotes) then
    ...
    return if ($wrap or count($content) > 1) then
        <div class="{ $config:css-content-class } { $class }">{ $content }</div>
    else
        $content
};
    ...
    return typeswitch ($node)
        ...
        case element() return
            if ($node/@class = "footnote") then (
                ...
                element {node-name($node)} { $node/@*, pages:clean-footnotes($node/node()) }
...
declare function pages:switch-view-id ($data as element()+, $view as xs:string) {
    let $root := if ($view = "div") then
        ($data/*[1][self::tei:pb], $data/preceding::tei:pb[1])[1]
    else
        (
            $data/ancestor::tei:div,
            $data/following::tei:div,
            $data/ancestor::tei:body,
            $data/ancestor::tei:front
        )[1]
    return $root
};
