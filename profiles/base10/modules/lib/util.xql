...
declare function tpu:parse-pi (
    ...
    let $default := map:merge(($defaultConfig, $newConfig), map {"duplicates": "use-last"})
    let $pis := map:merge(
        ...
        return if ($key = "view" and $value != $view) then (
    ...
    let $cfgOddAvail := if ($pis?odd) then
        doc-available($config:odd-root || "/" || $pis?odd)
    else
        false()
    let $pisWithOdd := if ($defaultConfig?overwrite) then
        if ($cfgOddAvail) then
            map:merge(
                ($default, map {"odd": $pis?odd, "output": $pis?output}),
                map {"duplicates": "use-last"}
            )
        else
    ...
    return map:merge(($default, $config), map {"duplicates": "use-last"})
};
