...
declare function facets:print-table (
    $config as map(*),
    $nodes as element()+,
    $values as xs:string*,
    $params as xs:string*
    ...
    let $count := if ($all) then
        50
    else
        $config?max
    let $facets := if ($config?max = 0) then
        map {}
    ...
    return if (map:size($facets) > 0) then
        <table>
            {
                array:for-each(
                    facets:sort($config, $lang, $facets),
                    function ($entry) {
                        map:for-each(
                            $entry,
                            function ($label, $freq) {
                                ...
                                if (empty($params)) then (
                                    ...
                                    let $nested := facets:print-table(
                                        $config,
                                        $nodes,
                                        ($values, head($params)),
                                        tail($params)
                                    )
                                    return if ($nested and head($params) eq $label) then
                                        <tr class="nested"><td colspan="2">{ $nested }</td></tr>
                                    else (
                                    )
                            }
                        )
...
declare function facets:display ($config as map(*), $nodes as element()+) {
    ...
    let $fcount := if ($config?max = 0) then (
    ) else
        map:size(
        ...
        <div class="facet-dimension" data-dimension="facet-{ $config?dimension }">
            {
                if ($config?max != 0) then
                    <h3>
                        <pb-i18n key="{ $config?heading }">{ $config?heading }</pb-i18n>
                        {
                            if ($fcount > $max) then
                                <label>
                                    <input
                                        class="facet"
                                        name="all-{ $config?dimension }"
                                        type="checkbox"
                                    >
                                        {
                                            if (
                                                facets:get-parameter("all-" || $config?dimension)
                                            ) then
                                                attribute checked { "checked" }
                                            else (
                                            )
...
declare function facets:get-parameter ($name as xs:string) {
    ...
    return if (exists($param)) then
        $param
    else
        let $fromSession := session:get-attribute($config:session-prefix || ".params")
        return if (exists($fromSession)) then
            $fromSession?($name)
        else (
        )
};

declare function facets:translate (
    $config as map(*)?,
    ...
    if (exists($config) and map:contains($config, "output")) then
        ...
        return if (function-arity($fn) = 2) then
            $fn($label, $language)
        else
            $fn($label)
    else
