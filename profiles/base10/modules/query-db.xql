...
declare function dbs:query-default (
    $fields as xs:string+,
    $query as xs:string,
        ...
        return switch ($field)
            case "head" return
                if (exists($target-texts)) then
                    ...
                    return $config:data-root!doc(. || "/" || $text)//db:title[ft:query(
                        .,
                        $query,
                    ...
                    collection(
                        $config:data-root
                    )//db:title[ft:query(., $query, query:options($sortBy))]
            default return
                if (exists($target-texts)) then
                    for $text in $target-texts
                    let $sections := $config:data-root!doc(. || "/" || $text)//db:section[ft:query(
                        .,
                        $query,
                        query:options($sortBy)
                    )]
                    return if (empty($sections)) then
                        $config:data-root!doc(. || "/" || $text)//db:article[ft:query(
                            .,
                            $query,
                            query:options($sortBy)
                        )]
                    ...
                    let $sections := collection($config:data-root)//db:section[ft:query(
                        .,
                        $query,
                        query:options($sortBy)
                    )]
                    return if (empty($sections)) then
                        collection($config:data-root)//db:article[ft:query(
                            .,
                            $query,
                            query:options($sortBy)
                        )]
...
declare function dbs:autocomplete ($doc as xs:string?, $fields as xs:string+, $q as xs:string) {
    ...
    return switch ($field)
        case "text" return
            if ($doc) then (
                doc($config:data-root || "/" || $doc)/util:index-keys-by-qname(
                    xs:QName("db:section"),
                    $q,
                ...
                collection($config:data-root)/util:index-keys-by-qname(
                    xs:QName("db:section"),
                    $q,
                    function ($key, $count) { $key },
                    30,
        ...
        case "head" return
            if ($doc) then
                ...
                collection(
                    $config:data-root
                )/util:index-keys-by-qname(xs:QName("db:title"), $q, function ($key, $count) { $key }, 30, "lucene-index")
        case "author" return
            collection($config:data-root)/ft:index-keys-for-field(
                $dbs:FIELD_PREFIX || "author",
                $q,
                function ($key, $count) { $key },
                30
            )
...
declare function dbs:query-metadata (
    $path as xs:string?,
    $field as xs:string?,
    $query as xs:string?,
    $sort as xs:string
    ...
    return <nav aria-label="breadcrumb">
        <ul>
            <li><a href="{ $parent-id }">{ $work-title }</a></li>
            {
                ...
                return <li>
                    <a
                        href="{ $parent-id }?action=search&amp;root={ $id }&amp;view={
                            $config?view
                        }&amp;odd={ $config?odd }"
                    >{ $parentDiv/db:title/string() }</a>
                </li>
            }
        </ul>
    </nav>
...
declare %private function dbs:query-default-view (
    $context as element()*,
    $query as xs:string,
    $fields as xs:string+
) {
    ...
    return switch ($field)
        ...
        default return
            $context[./descendant-or-self::db:section[ft:query(., $query, $query:QUERY_OPTIONS)]] |
                $context[./descendant-or-self::db:article[ft:query(
                    .,
                    $query,
                    $query:QUERY_OPTIONS
                )]]
