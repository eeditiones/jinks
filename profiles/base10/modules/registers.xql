...
declare function rapi:entry ($request as map(*)) {
    ...
    return if ($id) then
        if ($entry) then
            ...
            return collection($config:register-root)/id($entry-template)/child::*
    else
...
declare function rapi:save ($request as map(*)) {
    ...
    return if ($record) then (: update existing record :) (
        rapi:replace-entry($record, $data), $data, map {"status": "updated"}
    ) else (
        rapi:add-entry($data, $type), $data, map {"status": "created"}
    )
};

declare function rapi:add-entry ($record, $type) {
    let $target := rapi:insert-point($type)
    return update insert $record into $target
};
...
declare function rapi:prepare-record ($node as item()*, $resp, $type) {
    ...
    let $id := if ($node/@xml:id = $new) then
        rapi:next($type)
    else
        $node/@xml:id
...
declare function rapi:next ($type) {
    ...
    let $all-ids := switch ($type)
        case "place" return
            collection($config:register-root)/id($config?id)//tei:place[starts-with(
                @xml:id,
                $config?prefix
            )]/substring-after(@xml:id, $config?prefix)
        case "organization" return
            collection($config:register-root)/id($config?id)//tei:org[starts-with(
                @xml:id,
                $config?prefix
            )]/substring-after(@xml:id, $config?prefix)
        case "term" return
            collection($config:register-root)/id($config?id)//tei:category[starts-with(
                @xml:id,
                $config?prefix
            )]/substring-after(@xml:id, $config?prefix)
        case "work" return
            collection($config:register-root)/id($config?id)//tei:bibl[@type eq "work"][starts-with(
                @xml:id,
                $config?prefix
            )]/substring-after(@xml:id, $config?prefix)
        default return
            collection($config:register-root)/id($config?id)//tei:person[starts-with(
                @xml:id,
                $config?prefix
            )]/substring-after(@xml:id, $config?prefix)
    ...
    let $numeric-ids :=
        for $id in $all-ids
        return if ($id castable as xs:integer) then
            $id
        else (
        )
    let $last := if (count($all-ids)) then
        sort($numeric-ids)[last()]
    else
        1
    let $next := try { xs:integer($last) + 1 } catch * { 1 }
...
declare function rapi:query-register ($request as map(*)) {
    ...
    return array { rapi:query($type, $query) }
};
...
declare function rapi:query ($type as xs:string, $query as xs:string?) {
    try {
        switch ($type)
            case "place" return
                for $place in
                    collection($config:register-root)//tei:place[ft:query(tei:placeName, $query)]
                return map {
                    "id": $place/@xml:id/string(),
                    "label": $place/tei:placeName[@type = "main"]/string(),
                    "details":
                        ``[`{$place/tei:note/string()}` - `{$place/tei:country/string()}`, `{$place/tei:region/string()}`]``,
                    "link": $place/tei:ptr/@target/string()
                }
            case "person" return
                for $person in
                    collection($config:register-root)//tei:person[ft:query(tei:persName, $query)]
                let $birth := $person/tei:birth/tei:date/@when
                let $death := $person/tei:death/tei:date/@when
                let $dates := if ($birth) then
            ...
            case "organization" return
                for $org in
                    collection($config:register-root)//tei:org[ft:query(tei:orgName, $query)]
                return map {
            ...
            case "term" return
                for $term in
                    collection($config:register-root)//tei:taxonomy[ft:query(tei:category, $query)]
                return map {"id": $term/@xml:id/string(), "label": $term/tei:catDesc/string()}
            case "work" return
                for $bibl in
                    collection($config:register-root)//tei:bibl[ft:query(tei:title, $query)]
                return map {
...
declare function rapi:create-record ($type as xs:string, $id as xs:string, $data as map(*)) {
    switch ($type)
            ...
            >
                ...
                <placeName type="sort">{ $data?name }</placeName>
                {
                    if (exists($data?lat) and exists($data?lng)) then
                        <location><geo>{ string-join(($data?lat, $data?lng), " ") }</geo></location>
                    else (
                    )
                }
                ...
                <note />
                <ptr target="{ $data?links?1 }" type="geonames" />
                {
                    array:subarray($data?links, 2)
                        => array:for-each(
                            function ($link) {
                                <ptr
                                    xmlns="http://www.tei-c.org/ns/1.0"
            ...
            <person xmlns="http://www.tei-c.org/ns/1.0" xml:id="{ $id }">
                ...
                <note type="bio">{ $data?note }</note>
                {
                    if (exists($data?professionOrOccupation)) then
                        ...
                        return <occupation ref="{ $prof?id }">{ $prof?label }</occupation>
                    else (
                    )
                }
            </person>
        ...
        case "work" return
            <bibl xmlns="http://www.tei-c.org/ns/1.0" type="work" xml:id="{ $id }">
...
declare function rapi:normalize-gender ($values) {
    ...
    return switch ($gender?id)
        case "https://d-nb.info/standards/vocab/gnd/gender#male" return
            <gender xmlns="http://www.tei-c.org/ns/1.0" value="M">male</gender>
        case "https://d-nb.info/standards/vocab/gnd/gender#female" return
            <gender xmlns="http://www.tei-c.org/ns/1.0" value="F">female</gender>
        case "https://d-nb.info/standards/vocab/gnd/gender#notKnown" return
            <gender xmlns="http://www.tei-c.org/ns/1.0" value="U">unknown</gender>
        default return
            <gender xmlns="http://www.tei-c.org/ns/1.0" value="{ $gender?id }">
                { $gender?label }
            </gender>
};
...
declare function rapi:local-search-strings ($type as xs:string, $entry as element()?) {
    switch ($type)
        case "place" return
            $entry/tei:placeName/string()
        case "organization" return
            $entry/tei:orgName/string()
        case "term" return
            $entry/tei:catDesc/string()
        case "work" return
            $entry/tei:title/string()
        default return
            $entry/tei:persName/string()
};
...
declare function rapi:save-local-copy ($request as map(*)) {
    ...
    return if ($record) then
        ...
        let $target := rapi:insert-point($type)
        return if (sm:has-access(xs:anyURI(document-uri(root($target))), "w")) then (
            update insert $record into $target, map {"status": "updated"}
        ) else
        ...
        switch ($format)
            ...
            default return
                ...
                return map {
                    ...
                    "details":
                        <div>{ $pm-config:web-transform($entry, map {}, "annotations.odd") }</div>
                }
    else
