...
declare variable $query:QUERY_OPTIONS := map {"leading-wildcard": "yes", "filter-rewrite": "yes"};
...
declare %private function query:sort ($items as element()*, $sortBy as xs:string?) {
    let $items := if (exists($config:data-exclude)) then
        $items except $config:data-exclude
    else
        $items
    return if ($sortBy) then
        nav:sort($sortBy, $items)
    else
        $items
};

declare %private function query:dispatch (
    $config as map(*),
...
(:~
 ...
 :)
declare function query:query-default (
    $fields as xs:string+,
    $query as xs:string,
                ...
                "facets":
                    map:merge(
                        (
                            ...
                            return map {$dimension: request:get-parameter($param, ())},
                            for $dates in request:get-parameter("dates", ())
                            return map {"date": query:parse-date([], tokenize($dates, ";"))}
                        )
                    )
            },
            if ($sortBy) then
...
declare %private function query:parse-date ($array as array(*), $dates as xs:string*) {
    let $next := head($dates)
    return if ($next) then
        let $parts := if ($next = "?") then
            "1000"
        else
            tokenize($next, "-")
        return query:parse-date(array:append($array, $parts), tail($dates))
    else
        $array
};

declare function query:query-metadata (
    $root as xs:string?,
    $field as xs:string?,
    $query as xs:string?,
    $sort as xs:string
    ...
    let $mode := if (
        (empty($query) or $query = "") and
            empty(request:get-parameter-names()[starts-with(., "facet-")])
    ) then
        "browse"
    else
        "search"
    return map {"all": $results, "mode": $mode}
};
...
declare function query:get-breadcrumbs (
    $config as map(*),
    $hit as node(),
