...
(:~
 ...
 :)
declare variable $config:origin-whitelist := ("(?:https?://localhost:.*|https?://127.0.0.1:.*)");
...
(:~
 ...
 :)
declare variable $config:enable-proxy-caching := let $prop := util:system-property(
    "teipublisher.proxy-caching"
)
return exists($prop) and lower-case($prop) = "true";
...
(:~
 ...
 :)
declare variable $config:fop-config := let $fontsDir := config:get-fonts-dir()
            ...
            <fonts>
                {
                    if ($fontsDir) then (
                        <font
                            ...
                            kerning="yes"
                        ><font-triplet name="Junicode" style="normal" weight="normal" /></font>,
                        <font
                            embed-url="file:{ $fontsDir }/Junicode-Bold.ttf"
                            encoding-mode="single-byte"
                            kerning="yes"
                        ><font-triplet name="Junicode" style="normal" weight="700" /></font>,
                        <font
                            embed-url="file:{ $fontsDir }/Junicode-Italic.ttf"
                            encoding-mode="single-byte"
                            kerning="yes"
                        ><font-triplet name="Junicode" style="italic" weight="normal" /></font>,
                        <font
                            embed-url="file:{ $fontsDir }/Junicode-BoldItalic.ttf"
                            encoding-mode="single-byte"
                            kerning="yes"
                        ><font-triplet name="Junicode" style="italic" weight="700" /></font>
                    ) else (
                    )
                }
            </fonts>
...
(:
 ...
 : as working director.
 :)
declare variable $config:tex-temp-dir := util:system-property("java.io.tmpdir");

(:~
 : Configuration for epub files.
 :)
declare variable $config:epub-config := function (

    $doc as document-node(),
    $langParameter as xs:string?
) {
    let $root := $doc/*
...
declare variable $config:register-map := map {
    "person": map {"id": "pb-persons", "default": "person-default", "prefix": "person-"},
    "place": map {"id": "pb-places", "default": "place-default", "prefix": "place-"},
    "bibliography": map {"id": "pb-bibl", "default": "bibl-default", "prefix": "bibl-"},
    "organization":
        map {"id": "pb-organizations", "default": "organization-default", "prefix": "org-"},
    "term": map {"id": "pb-keywords", "default": "term-default", "prefix": "category-"},
    "work": map {"id": "pb-works", "default": "work-default", "prefix": "work-"}
};
    ...
    "memberCollections":
        (
            map {
                ...
                "members":
                    function () {
                        nav:get-root((), map {"leading-wildcard": "yes", "filter-rewrite": "yes"})
                    },
                "metadata":
                    function ($doc as document-node()) {
                        let $properties := tpu:parse-pi($doc, ())
                        return map:merge(
                            (
                                map:entry(
                                    "title",
                                    nav:get-metadata($properties, $doc/*, "title")/string()
                                ),
                                map {
                                    "dts:dublincore":
                                        map {
                                            "dc:creator":
                                                string-join(
                                                    nav:get-metadata($properties, $doc/*, "author"),
                                                    "; "
                                                ),
                                            "dc:license":
                                                nav:get-metadata($properties, $doc/*, "license")
                                        }
                                }
                            )
                        )
            ...
            map {
                ...
                "members": function () { collection($config:odd-root)/tei:TEI },
                "metadata":
                    function ($doc as document-node()) {
                        map {
                            "title":
                                string-join(
                                    $doc//tei:teiHeader/tei:fileDesc/tei:titleStmt/tei:title[not(
                                        @type
                                    )],
                                    "; "
                                )
...
declare function config:get-document ($idOrName as xs:string) {
    let $document := if ($config:address-by-id) then
    ...
    return if (count($document)) then
        $document
    else
        (: if document not found, try to find it again in any way: by id, or by path in data-root or data-default :)
        head(
            (
                collection($config:data-root)/id($idOrName),
                doc(xmldb:encode-uri($idOrName)),
                doc(xmldb:encode-uri($config:data-root || "/" || $idOrName)),
                doc(xmldb:encode-uri($config:data-default || "/" || $idOrName))
            )
        )
};
...
declare function config:get-relpath ($node as node(), $root as xs:string) {
    let $root := if (ends-with($root, "/")) then
        $root
    else
        $root || "/"
    return substring-after(document-uri(root($node)), $root)
};
...
declare function config:get-data-dir () as xs:string? {
    try {
        let $request := <http:request
            href="http://localhost:{ request:get-server-port() }/{
