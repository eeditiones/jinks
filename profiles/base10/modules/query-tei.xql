...
declare function teis:query-default (
    $fields as xs:string+,
    $query as xs:string,
        ...
        return switch ($field)
            case "head" return
                if (exists($target-texts)) then
                    ...
                    return $config:data-default!doc(. || "/" || $text)//tei:head[ft:query(
                        .,
                        $query,
                    ...
                    collection(
                        $config:data-default
                    )//tei:head[ft:query(., $query, query:options($sortBy))]
            default return
                if (exists($target-texts)) then
                    for $text in $target-texts
                    let $divisions := $config:data-default!doc(. || "/" || $text)//tei:div[ft:query(
                        .,
                        $query,
                        query:options($sortBy)
                    )]
                    return if (empty($divisions)) then
                        $config:data-default!doc(. || "/" || $text)//tei:text[ft:query(
                            .,
                            $query,
                            query:options($sortBy)
                        )]
                    ...
                    let $divisions := collection($config:data-default)//tei:div[ft:query(
                        .,
                        $query,
                        query:options($sortBy)
                    )]
                    return if (empty($divisions)) then
                        collection($config:data-default)//tei:text[ft:query(
                            .,
                            $query,
                            query:options($sortBy)
                        )]
...
declare function teis:query-metadata (
    $path as xs:string?,
    $field as xs:string?,
    $query as xs:string?,
    $sort as xs:string
) {
    let $queryExpr := if ($field = "file" or empty($query) or $query = "") then
        "file:*"
    else
        ($field, "text")[1] || ":" || $query
    let $options := query:options($sort, ($field, "text")[1])
    let $result := $config:data-default!(
        collection(. || "/" || $path)//tei:text[ft:query(., $queryExpr, $options)]
    )
    return query:sort($result, $sort)
};
...
declare function teis:autocomplete ($doc as xs:string?, $fields as xs:string+, $q as xs:string) {
    ...
    return switch ($field)
        case "author" return
            collection($config:data-default)/ft:index-keys-for-field(
                "author",
                $lower-case-q,
                function ($key, $count) { $key },
                30
            )
        case "file" return
            collection($config:data-default)/ft:index-keys-for-field(
        ...
        case "text" return
            if ($doc) then (
                doc($config:data-default || "/" || $doc)/util:index-keys-by-qname(
                    xs:QName("tei:div"),
                    $lower-case-q,
                    function ($key, $count) { $key },
                    30,
                ...
                collection($config:data-default)/util:index-keys-by-qname(
                    xs:QName("tei:div"),
                    $lower-case-q,
                    function ($key, $count) { $key },
                    30,
        ...
        case "head" return
            if ($doc) then
                ...
                collection(
                    $config:data-default
                )/util:index-keys-by-qname(xs:QName("tei:head"), $lower-case-q, function ($key, $count) { $key }, 30, "lucene-index")
        default return
            collection($config:data-default)/ft:index-keys-for-field(
                "title",
                $lower-case-q,
            ...
            <li><a href="{ $parent-id }">{ $work-title }</a></li>
            {
                ...
                let $id := util:node-id(
                    if ($config?view = "page") then
                        ($parentDiv/preceding::tei:pb[1], $parentDiv)[1]
                    else
                        $parentDiv
                )
                return <li>
...
declare function teis:expand ($data as node()) {
    ...
    let $div := if ($data instance of element(tei:pb)) then
        let $nextPage := $data/following::tei:pb[1]
        return if ($nextPage) then
            if ($field = "text") then
                (
                ...
                $data/ancestor::tei:text
        else (: if there's only one pb in the document, its whole
                      text part should be returned :) if (
            count($data/ancestor::tei:text//tei:pb) = 1
        ) then (
            $data/ancestor::tei:text
        ) else
    ...
    let $expanded := if (exists($result)) then
        util:expand($result, "add-exist-id=all")
    else
        $div
    return if ($data instance of element(tei:pb)) then
        $expanded//tei:pb[@exist:id = util:node-id($data)]
    else
        $expanded
};

declare %private function teis:query-default-view (
    $context as element()*,
    $query as xs:string,
    $fields as xs:string+
) {
...
declare function teis:get-current ($config as map(*), $div as node()?) {
    if (empty($div)) then (
    ) else if ($div instance of element(tei:teiHeader)) then
        $div
    else
