...
declare function jats:query-default (
    $fields as xs:string+,
    $query as xs:string,
    ...
    if (string($query)) then
        ...
        return switch ($field)
            ...
            default return
                if (exists($target-texts)) then
                    for $text in $target-texts
                    let $divisions := $config:data-root!doc(. || "/" || $text)//sec[ft:query(
                        .,
                        $query,
                        query:options($sortBy)
                    )]
                    return if (empty($divisions)) then
                        $config:data-root!doc(. || "/" || $text)//body[ft:query(
                            .,
                            $query,
                            query:options($sortBy)
                        )]
                    ...
                    let $divisions := collection($config:data-root)//sec[ft:query(
                        .,
                        $query,
                        query:options($sortBy)
                    )]
                    return if (empty($divisions)) then
                        collection($config:data-root)//body[ft:query(
                            .,
                            $query,
                            query:options($sortBy)
                        )]
...
declare function jats:query-metadata (
    $path as xs:string?,
    $field as xs:string?,
    $query as xs:string?,
    $sort as xs:string
...
declare %private function jats:query-default-view (
    $context as node()*,
    $query as xs:string,
    $fields as xs:string+
) {
