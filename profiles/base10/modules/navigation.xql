...
declare %private function nav:dispatch (
    $config as map(*),
...
declare function nav:output-footnotes ($footnotes as element()*) {
    <div class="popovers">{ $footnotes/self::pb-popover }</div>,
    <div class="footnotes">{ $footnotes[not(self::pb-popover)] }</div>
};
            ...
            <summary>
                {
                    if ($context?xmlId) then
                        <pb-link
                            ...
                            xml-id="{ $context?xmlId }"
                        >{ $context?label }</pb-link>
                    else
                        <pb-link
                            emit="{ $context?target }"
                            node-id="{ $context?nodeId }"
