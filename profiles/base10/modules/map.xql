...
declare function mapping:cortez-translation ($root as element(), $userParams as map(*)) {
    let $first := (
        ($root/following-sibling::text()/ancestor::*[@xml:id])[last()],
        $root/following-sibling::*[@xml:id],
        ($root/ancestor::*[@xml:id])[last()]
    )[1]
    ...
    return nav:milestone-chunk($mappedStart, $mappedEnd, $context)
};
...
declare function mapping:barum-book ($root as element(), $userParams as map(*)) {
    ...
    let $node := root($root)//tei:text[@type = "translation"]//tei:div[@type = "book"][@n =
        $bookNumber]
...
declare function mapping:offset-translation ($root as element(), $userParams as map(*)) {
    ...
    let $translation-root := util:node-id(
        root($root)//tei:text[@type = "translation"][@xml:lang = $language]/tei:body
    )
...
declare function mapping:suffix-translation ($root, $userParams as map(*)) {
    let $language := head(
        (
            if ($userParams?lang) then
                $userParams?lang
            else (
            ),
            if ($userParams?language) then
                $userParams?language
            else (
            ),
    ...
    return if (matches($language, "^pl*")) then
        doc(
            util:collection-name($root) ||
                "/" ||
                replace(util:document-name($root), "^(.+)\.xml$", "$1") ||
                "-pl.xml"
        )
    else
        $root
