...
declare function browse:parent-link ($context as map(*)) {
    let $parts := if (exists($context?doc)) then
    ...
    return string-join(subsequence($parts, 1, count($parts) - 1), "/")
};
            ...
            map {
                ...
                "doc":
                    if ($config:address-by-id) then
                        config:get-identifier($doc)
                    else
                        $config:context-path || "/" || $config?relpath,
...
declare function browse:show-hits ($context as map(*), $doc as element()) {
    if (exists($context?request?parameters?query) and $context?request?parameters?query != "") then
        ...
        return if (count($matches)) then
            <div class="matches">
                <div class="count">
                    <pb-i18n key="browse.items" options='{{"count": { count($matches) }}}' />
                </div>
                {
                    for $match in subsequence($matches, 1, 5)
                    let $config := <config table="no" width="60" />
                    return kwic:get-summary($field, $match, $config)
                }
            </div>
        else (
        )
