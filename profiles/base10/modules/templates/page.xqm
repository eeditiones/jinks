...
declare variable $page:EXIDE := let $pkg := collection(repo:get-root())//expath:package[@name =
    "http://exist-db.org/apps/eXide"]
let $appLink := if ($pkg) then
    substring-after(util:collection-name($pkg), repo:get-root())
else (
)
let $path := string-join(
    (request:get-context-path(), request:get-attribute("$exist:prefix"), $appLink, "index.html"),
    "/"
)
return replace($path, "/+", "/");
...
declare function page:parameter ($context as map(*), $name as xs:string, $default as item()*) {
    ...
    return if (exists($reqParam)) then
        $reqParam
    else
        $default
};
...
declare %private function page:primary-lang ($tag as xs:string?) as xs:string? {
    if (not(normalize-space($tag))) then (
        ...
        return lower-case(head(tokenize($before-hyphen, "_")))
};

(:~
 : First Accept-Language preference that matches a supported code (order preserved).
 :)
declare %private function page:lang-from-accept-language (
    $header as xs:string?,
...
declare function page:resolve-language ($context as map(*)) as xs:string? {
    ...
    let $fromHeader := page:lang-from-accept-language(
        request:get-header("Accept-Language"),
        $supported
    )
    return if (string-length(normalize-space($param)) gt 0) then
...
declare function page:collection-breadcrumbs ($context as map(*)) {
    if (exists($context?doc)) then
        let $components := config:get-relpath($context?doc?content, $config:data-default)
            => tokenize("/")
        return if (count($components) = 1) then
            <li>
                <a href="{ $context?context-path }/{ $context?defaults?browse }?collection=">
                    <pb-i18n
                        key="breadcrumb.document-root"
                    >
                            Home
                        </pb-i18n>
                </a>
            ...
            return <li>
                <a
                    href="{ $context?context-path }/{ $context?defaults?browse }?collection={
                        string-join(subsequence($components, 1, $i), "/")
                    }"
                >
...
(:~
 ...
 :)
declare function page:transform (
    $nodes as node()*,
    $parameters as map(*)?,
    $odd as xs:string?,
    $mapFunction as xs:string?
) {
    let $mappingFun := if ($mapFunction) then
        function-lookup(xs:QName("mapping:" || $mapFunction), 2)
    else (
    )
    let $odd := head(($odd, $config:default-odd))
    for $node in $nodes
    let $mapped := if (exists($mappingFun)) then
    ...
    return $pm-config:web-transform($mapped, $params, $odd)
};
