...
declare function local:allowOriginDynamic ($origin as xs:string?) {
    ...
    return if (local:checkOriginWhitelist($config:origin-whitelist, $origin)) then
        $origin
    else
        "*"
};
...
(: static resources from the resources, transform, templates, odd or modules subirectories are directly returned :)
    ...
    matches($exist:path, "^.*/(resources|transform|templates)/.*$") or
        matches($exist:path, "^.*/odd/.*\.css$") or
        $exist:path eq "/robots.txt" or
        matches($exist:path, "^.*/modules/.*\.json$")
) then
    ...
    return <dispatch xmlns="http://exist.sourceforge.net/NS/exist">
        <forward
            url="{ $exist:controller }/{ $dir }/{
                substring-after($exist:path, "/" || $dir || "/")
            }"
        >
            {
...
(: all other requests are passed on the Open API router :)
    ...
    let $main := if (matches($exist:path, "^/+api/+(?:odd|lint)")) then
        "api-odd.xql"
    else if (
        matches($exist:path, "/+tex$") or
            matches($exist:path, "/+api/+(?:actions/reindex|actions/file-sync)$")
    ) then
        "api-dba.xql"
    else
        "api.xql"
