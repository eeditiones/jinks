        ...
        it("displays metadata panel when enabled", () => {
            ...
            cy.get("body").then(($body) => {
                const hasMetadataPanel =
                    $body.find(
                        '[class*="metadata"], aside[class*="metadata"], pb-view[mode="metadata-panel"]',
                    ).length > 0 ||
                ...
                if (hasMetadataPanel) {
                    cy.get('[class*="metadata"], aside[class*="metadata"]').should("exist");
                } else {
    ...
    describe("Metadata Content", () => {
        it("displays document metadata when available", () => {
            ...
            cy.get("body").then(($body) => {
                ...
                if (hasMetadata) {
                    ...
                    cy.get('[class*="metadata"], aside').should("be.visible");
                } else {
        ...
        it("shows teiHeader information", () => {
            ...
            cy.get("body").then(($body) => {
                ...
                const hasHeaderInfo =
                    text.includes("Critik der reinen Vernunft") || // Title from demo doc
                    $body.find('[class*="metadata"]').length > 0;
            ...
            cy.get("body").then(($body) => {
                ...
                if (metadataPanel.length > 0) {
                    ...
                    const hasStructuredContent =
                        metadataPanel.find("dl, dt, dd, .field, [data-field]").length > 0 ||
                        metadataPanel.text().length > 0;

                    // Metadata should have some content
                    if (hasStructuredContent) {
                        cy.get('[class*="metadata"]').should("not.be.empty");
                    }
                } else {
    ...
    describe("Metadata Panel Visibility", () => {
        it("respects metadata panel feature enable/disable", () => {
            ...
            cy.get("body").then(($body) => {
                ...
                const hasMetadata =
                    $body.find('[class*="metadata"], pb-view[mode="metadata-panel"]').length > 0;
