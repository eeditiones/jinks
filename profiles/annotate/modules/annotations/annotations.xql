...
declare function anno:find-references ($request as map(*)) {
    map:merge(
        ...
        return map:entry($id, count($matches))
    )
};
...
declare function anno:save ($request as map(*)) {
    ...
    return if ($annotations instance of array(*)) then
        let $path := xmldb:decode($request?parameters?path)
        let $srcDoc := config:get-document($path)
        return anno:merge-and-save($srcDoc, $path, $annotations, $body?log)
    else
        let $result :=
            for $path in map:keys($annotations)
            let $srcDoc := config:get-document($path)
            return anno:merge-and-save($srcDoc, $path, $annotations($path), $body?log)
        return router:response(200, count(map:keys($annotations)) || " documents merged")
};

declare function anno:merge-and-save (
    $srcDoc as node(),
    $path as xs:string,
    ...
    return if (not($hasAccess) and request:get-method() = "PUT") then
        ...
        let $map := map:merge(
            ...
            let $ordered :=
                ...
                return $anno
            return map:entry($id, anno:apply($node, $ordered))
        )
        let $merged := anno:merge($doc, $map) => anno:strip-exist-id() => anno:extend-header($log)
        let $output := document { $srcDoc/(processing-instruction() | comment()), $merged }
        let $serialized := serialize($output, map {"indent": false()})
        let $stored := if (request:get-method() = "PUT") then
            xmldb:store(util:collection-name($srcDoc), util:document-name($srcDoc), $serialized)
        else (
        )
        return map {"content": $serialized, "changes": array { $map?*!anno:strip-exist-id(.) }}
    else
...
declare %private function anno:strip-exist-id ($nodes as node()*) {
    ...
    return typeswitch ($node)
        case document-node() return
            document { anno:strip-exist-id($node/node()) }
        case element(exist:delete) return
            anno:strip-exist-id($node/node())
        case element() return
            element {node-name($node)} {
                $node/@* except $node/@exist:*, anno:strip-exist-id($node/node())
            }
...
declare %private function anno:extend-header ($nodes as node()*, $log as map(*)?) {
    ...
    return typeswitch ($node)
        case document-node() return
            document { anno:extend-header($node/node(), $log) }
            ...
            return element {node-name($node)} {
                ...
                if (not($node/tei:standOff) and $notes) then
                    <standOff xmlns="http://www.tei-c.org/ns/1.0">
                        <listAnnotation>{ $notes }</listAnnotation>
                    </standOff>
                else (
                )
            }
        ...
        case element(tei:revisionDesc) return
            if (not($node/tei:listChange)) then
                ...
                element {node-name($node)} { $node/@*, anno:extend-header($node/node(), $log) }
...
declare %private function anno:merge ($nodes as node()*, $elements as map(*)) {
    ...
    return typeswitch ($node)
        ...
        case element() return
            let $replacement := if ($node/@exist:id) then
                $elements($node/@exist:id)
            else (
            )
            return if ($replacement) then
                if ($node instance of element(exist:delete)) then
                    ...
                    element {node-name($replacement)} {
                        $replacement/@*, anno:merge($replacement/node(), $elements)
                    }
            else
                element {node-name($node)} { $node/@*, anno:merge($node/node(), $elements) }
...
declare %private function anno:apply ($node, $annotations) {
    if (empty($annotations)) then
        ...
        return if ($anno?type = "modify") then
            ...
            return anno:apply($output, tail($annotations))
    ...
    return typeswitch ($node)
        case element(tei:lem) return
            if ($target is $node/..) then
                element exist:delete { $node/@*, anno:delete($node/node(), $target) }
            else
                element {node-name($node)} { $node/@*, anno:delete($node/node(), $target) }
...
declare %private function anno:modify (
    $nodes as node()*,
    $target as node(),
    $annotation as map(*)
) {
    ...
    return typeswitch ($node)
        case element(tei:choice)|element(tei:app) return
            element {node-name($node)} { $node/@*, anno:modify($node/node(), $target, $annotation) }
        case element(tei:rdg) return
            if ($node/.. is $target) then
                ...
                element {node-name($node)} {
                    $node/@*, anno:modify($node/node(), $target, $annotation)
                }
        case element(tei:expan)|element(tei:corr)|element(tei:reg) return
            if ($node/.. is $target) then
                element {node-name($node)} {
                    $node/@*, text { $annotation?properties(local-name($node)) }
                }
            else if ($node is $target) then
                element {node-name($node)} {
                    map:for-each(
...
declare %private function anno:apply (
    $node as node(),
    $startOffset as xs:int,
    $endOffset as xs:int,
    $annotation as map(*)
) {
    let $start := anno:find-offset(
        $node,
        $startOffset,
        "start",
        $node instance of element(tei:note)
    )
    let $end := anno:find-offset($node, $endOffset, "end", $node instance of element(tei:note))
    let $startAdjusted := if ($start?2 = 1 and not($start?1 is $end?1)) then
        [anno:find-outermost($node, $start?1, "start"), 1]
    else
        $start
    let $endAdjusted := if ($end?2 = string-length($end?1) and not($start?1 is $end?1)) then
        let $outer := anno:find-outermost($node, $end?1, "end")
        let $offset := if ($outer/following-sibling::node()) then
            1
        else
            $end?2
        return [anno:find-outermost($node, $end?1, "end"), $offset]
    else
        $end
    return anno:transform($node, $startAdjusted, $endAdjusted, false(), $annotation)
};

declare %private function anno:find-outermost (
    $context as node(),
    $node as node(),
    $pos as xs:string
) {
    let $parent := $node/..
    return if ($parent is $context) then
        $node
    else if (
        ($pos = "start" and empty($parent/preceding-sibling::node())) or
            ($pos = "end" and empty($parent/following-sibling::node()))
    ) then
...
declare %private function anno:find-offset (
    $nodes as node()*,
    $offset as xs:int,
    $pos as xs:string,
    $isNote as xs:boolean?
) {
    if (empty($nodes)) then (
    ) else
        let $node := head($nodes)
        return typeswitch ($node)
            ...
            case element(tei:note) return
                if ($isNote) then
                    ...
                    return if (exists($found)) then
                        ...
                        anno:find-offset(
                            tail($nodes),
                            $offset - anno:string-length($node),
                            $pos,
                            ()
                        )
                else
            ...
            case element() return
                ...
                return if (exists($found)) then
                    ...
                    anno:find-offset(tail($nodes), $offset - anno:string-length($node), $pos, ())
            case comment() return
                anno:find-offset(tail($nodes), $offset - string-length($node), $pos, ())
            case text() return
                let $len := string-length($node)
                return if ($offset <= $len) then
                    [$node, $offset]
...
declare %private function anno:string-length ($nodes as node()*, $length as xs:int) {
    if ($nodes) then
        ...
        return anno:string-length(tail($nodes), $newLength)
    else
        $length
};

declare %private function anno:transform (
    $nodes as node()*,
    $start,
    $end,
    $inAnno,
    ...
    return typeswitch ($node)
        case element() return
            ...
            (: called inside the annotation being processed? :)
                ...
                if ($node >> $end?1) then (
                    ...
                    element {node-name($node)} {
                        $node/@*, anno:transform($node/node(), $start, $end, $inAnno, $annotation)
                    }
        ...
        case text() return
            if ($node is $start?1) then (
                ...
                ($inAnno and $node >> $end?1) or
                    (not($inAnno) and $node >> $start?1 and $node << $end?1)
            ) then (
            ) else
                $node
...
declare function anno:form-template ($request as map(*)) {
    util:log(
        "info",
        (
            "form-template: " || $request?parameters?id,
