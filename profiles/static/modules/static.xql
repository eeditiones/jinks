...
declare variable $static:ERROR_PART_LOAD_FAILED := QName(
    "http://tei-publisher.com/jinks/static",
    "part-load-failed"
);

declare variable $static:ERROR_LOAD_FAILED := QName(
    "http://tei-publisher.com/jinks/static",
    "load-failed"
);
...
declare function static:preload-view ($context as map(*), $config as map(*)) {
    ...
    return static:save-index($path || "/index.json", $entries)
};

declare %private function static:preload-view-next (
    $context as map(*),
    $config as map(*),
    $root as xs:string?,
...
declare function static:save-index ($path as xs:string, $entries as map(*)*) {
    let $oldMap := if (util:binary-doc-available($path)) then
...
(:~
 ...
 :)
declare function static:paginate (
    $context as map(*),
    $config as array(*),
...
(:~
 ...
 :)
declare %private function static:next-page (
    $context as map(*),
    $parts as array(map(*)+),
    $root as xs:string?,
    $template as xs:string,
    ...
    let $json := map:merge(
        (
            ...
            return map:entry(head(($part?id, "default")), $data)
        )
    )
    let $templateContent := cpy:resource-as-string($context, $template)?content
    let $output := tmpl:process(
        $templateContent,
        map:merge(($context, map {"pagination": map {"page": $count}, "parts": $json})),
        map {"plainText": false(), "resolver": cpy:resource-as-string($context, ?)}
    )
    let $targetPath := path:resolve-path($context?target, $targetPathGen($context, $count))
    let $nil := (
        path:mkcol($context, $targetPath),
        xmldb:store($targetPath, "index.html", $output, "text/html")
    )
    return (
        map {
...
declare %private function static:load-part (
    $context as map(*),
    $path as xs:string,
    $params as map(*)
) {
    let $mergedParams := map:merge(
        (
            map {"view": $context?defaults?view, "odd": $context?defaults?odd, "serialize": "xml"},
            for $param in map:keys($params)[not(. = ("path", "id"))]
            return map:entry($param, $params($param))
        )
    )
    let $urlParams := static:params-to-query($mergedParams)
    let $request := <http:request
        href="{ $context?base-uri }/api/parts/{ encode-for-uri($path) }/json?wrap=false&amp;{
            $urlParams
        }"
        method="GET" />
    ...
    return if ($response[1]/@status = 200) then
        ...
        return map:merge(($parsed, map {"key": static:compute-key($mergedParams)}))
    else
...
declare %private function static:params-to-query ($params as map(*)) {
    string-join(
        ...
        return concat($key, "=", encode-for-uri($params($key))),
        "&amp;"
    )
};
...
declare function static:load ($context as map(*)?, $url as xs:string, $target as xs:string?) {
    ...
    return if ($response[1]/@status = 200) then (
        ...
        return if ($target) then (
            map {"type": "static:load", "message": $url},
            let $targetPath := if (exists($context)) then
                path:resolve-path($context?target, $target)
            else
                $target
...
declare %private function static:strip-namespaces ($nodes as node()*) {
    ...
    return typeswitch ($node)
        case element() return
            element {local-name($node)} { $node/@*, static:strip-namespaces($node/node()) }
...
(:~
 ...
 :)
declare function static:split (
    $context as map(*),
    $input as item()*,
    $batchSize as xs:integer,
    $template as xs:string,
    $targetPathGen as function(*)
) {
    let $chunks :=
    ...
    return static:split($context, $chunks, $template, $targetPathGen)
};
...
declare function static:split (
    ...
    let $output := tmpl:process(
        ...
        map:merge(
            (
                $context,
                map {
                    "pagination": map {"page": $chunk?page, "all": $chunks},
                    "content":
                        ...
                        return map:merge(
                            (
                                ...
                                map {
                                    "content":
                                        let $contentChunks := if (
                                            $chunk?content instance of array(*)
                                        ) then
                                            $chunk?content
                                        else
                                            array { $chunk?content }
                                        for $content in $contentChunks?*
                                        return parse-xml-fragment($content)
                                }
                            )
                        )
                }
    ...
    return (
        map {"type": "static:split", "message": "Writing " || $targetPath},
        path:mkcol($context, $targetPath),
        xmldb:store($targetPath, "index.html", $output, "text/html")[2]
    )
};
...
declare function static:index ($context as map(*), $input as item()*, $collectionConfig as map(*)) {
    if ($collectionConfig?index) then
        let $lines :=
            for $doc in $input
            let $href := $context?base-uri ||
                "/api/static/" ||
                encode-for-uri($doc?path) ||
                "/" ||
                $collectionConfig?index ||
                "?prefix=" ||
                $collectionConfig?path-prefix
            let $request := <http:request href="{ $href }" method="GET" />
            let $response := http:send-request($request)
            return if ($response[1]/@status = 200) then
                let $data := util:binary-to-string(xs:base64Binary($response[2]))
                return parse-json($data)?*!serialize(., map {"method": "json", "indent": false()})
            else
        ...
        let $current-contents := if (util:binary-doc-available($jsonPath)) then
            unparsed-text-lines($jsonPath)
        else (
        )
        let $new-contents := string-join(($current-contents, $lines), "&#10;")
        return xmldb:store($context?target, "index.jsonl", $new-contents, "application/jsonl")[2]
    else (
    )
};
...
(:~
 ...
 :)
declare function static:redirect (
    $context as map(*),
    $target as xs:string,
    $redirectTo as xs:string
) {
    ...
    return xmldb:store($targetPath, "index.html", $html, "text/html")[2]
};
...
declare %private function static:fix-links ($nodes as node()*, $links as map(*)) {
    ...
    return typeswitch ($node)
        case document-node() return
            document { static:fix-links($node/node(), $links) }
        case element(a)|element(xhtml:a) return
            if (starts-with($node/@href, "?id=")) then
                ...
                return element {node-name($node)} {
                    $node/@* except $node/@href, attribute href { $resolved }, $node/node()
                }
            else
                element {node-name($node)} { $node/@*, static:fix-links($node/node(), $links) }
        case element(pb-link)|element(xhtml:pb-link) return
            let $target := if ($node/@node-id) then
                $links("exist-" || $node/@node-id)
            else if ($node/@xml-id) then
                $links($node/@xml-id)
            else (
            )
            return <a href="{ $target }">{ $node/node() }</a>
        case element() return
            element {node-name($node)} { $node/@*, static:fix-links($node/node(), $links) }
...
declare function static:fix-links ($context as map(*)) {
    ...
    let $targets := map:merge(
        for $node in collection($context?target)/html//@id
        return map:entry(
            $node,
            $context?context-path ||
                "/" ||
                document-uri(root($node)) => substring-after($base || "/")
        )
    )
    for $doc in collection($context?target)
    let $modified := static:fix-links($doc, $targets)
    return xmldb:store(util:collection-name($doc), util:document-name($doc), $modified)[2]
};

declare function static:prepare ($jsonConfig as map(*)) {
    let $staticTarget := if ($jsonConfig?static?target) then
    ...
    let $baseUri := request:get-scheme() ||
        "://" ||
    ...
    let $context := map:merge(
        (
            ...
            map {
                ...
                "defaults": map:merge(($jsonConfig?defaults, map:entry("browse", "")))
            }
        )
    )
    let $_ := (
        if (xmldb:collection-available($context?target)) then
...
declare %private function static:generate-collections-from-config ($context as map(*)) {
    map:for-each(
        ...
        function ($collection, $config) {
            ...
            let $docsAll := if ($collection = "") then
                static:load($context?base-uri || "/api/documents?link=" || $link)
            else
                static:load(
                    $context?base-uri || "/api/documents/" || $collection || "?link=" || $link
                )
            let $docs := if (
                map:contains($config, "include") and $config?include instance of array(*)
            ) then
                array:filter($docsAll, function ($doc) { $doc?path = $config?include })
            else
                $docsAll
            let $_ := util:log(
                "INFO",
                (
                    "<static> Processing " || count($docs?*) || " documents in collection ",
                    $collection
                )
            )
            return (
                ...
                static:split(
                    ...
                    10,
                    $config?template,
                    function ($context as map(*), $page as xs:int) {
                        path:resolve-path($collection, $page)
                    }
                ),
                for $doc in $docs?*
                let $toc := if ($config?paginate?toc) then
                    static:load(
                        $context?base-uri ||
                            "/api/document/" ||
                            encode-for-uri($doc?path) ||
                ...
                let $docContext := map:merge(($context, map {"table-of-contents": $toc}))
                        ...
                        array
                            {
                                ...
                                return map:merge(($part, map:entry("path", $doc?path)))
                            },
...
declare function static:generate-from-config ($context as map(*)) {
    ...
    return cpy:copy-resource($context, $css, $css),
    static:fix-links($context),
    if (map:contains($context?static, "redirect")) then
        map:for-each(
            $context?static?redirect,
            function ($source, $target) { static:redirect($context, $source, $target) }
        )
    else (
    )
};
...
declare function static:copy ($context as map(*)) {
    ...
    return cpy:copy-collection($context, $operation?from, $operation?to, $operation?filter)
};
...
declare function static:fetch ($context as map(*), $collectionConfig as map(*), $doc as map(*)) {
    ...
    let $target := static:expand-parameter(
        $fetch?target,
        map:merge(($context, map:entry("doc", $doc)))
    )
    let $log := util:log("INFO", ("<static> Fetching ", $url, " to ", $target))
    return static:load($context, $url, $target)
};
...
declare function static:expand-parameter ($parameter as xs:string, $context as map(*)) {
    tmpl:process(
        $parameter,
        $context,
        map {"plainText": true(), "resolver": cpy:resource-as-string($context, ?)}
    )
};
