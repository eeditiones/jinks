...
declare function sg:generate-static-zip ($request as map(*)) {
    ...
    return response:stream-binary($xar, "media-type=application/zip", $name || ".static.zip")
};
    ...
    sg:scan(
        ...
        function ($collection as xs:anyURI, $resource as xs:anyURI?) {
            if (exists($resource)) then
                ...
                return if (starts-with($relative-path, "transform/")) then (
                    ...
                    let $options := if ($mime = "text/html") then
                        map {"method": "html", "indent": false()}
                    else
                        map {"indent": false()}
                    return <entry name="{ $relative-path }" type="text">
...
declare %private function sg:scan (
    $root as xs:anyURI,
    ...
    if (sm:has-access($root, "rx")) then
        ...
        return $func($root, xs:anyURI($root || "/" || $child))
    else (
    ),
    if (sm:has-access($root, "rx")) then
        for $child in xmldb:get-child-collections($root)
        return sg:scan(xs:anyURI($root || "/" || $child), $func)
    else (
    )
};
