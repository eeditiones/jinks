...
async function setupFacsimileIntegration(editor, pbFacsimile, baseUri, doc) {
    const result = await fetch(`${baseUri}/api/facsimiles/${encodeURIComponent(doc)}`);
    if (!result.ok) {
        return;
    }
    const facsimileByFacs = await result.json();
    pbFacsimile.setAttribute("facsimiles", JSON.stringify(Object.values(facsimileByFacs)));
