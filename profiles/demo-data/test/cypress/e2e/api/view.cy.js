...
describe("/{doc}", () => {
    it("Should retrieve matching view template", () => {
        cy.request("/demo%2Flet695.xml").its("status").should("eq", 200);
    ...
    it("fails to load template for non-existing document", () => {
        cy.request({ url: "/foo.xml", failOnStatusCode: false }).its("status").should("eq", 200);
...
describe("/{file}.html", () => {
    it("Should retrieve HTML file", () => {
        cy.request("/collection.html").its("status").should("eq", 200);
    ...
    it("fails to load HTML file", () => {
        cy.request({ url: "/foo.html", failOnStatusCode: false }).its("status").should("eq", 200);
