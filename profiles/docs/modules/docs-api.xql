...
declare function api:lemmata ($request as map(*)) {
    ...
    let $entries := if ($search and $search != "") then
        doc($config:data-root || "/demo/Kirchner-Michaelis-1907.xml")//tei:entry[ft:query(
            .,
            "lemma:(" || $search || "*)",
            map {"leading-wildcard": "yes", "filter-rewrite": "yes", "fields": "lemma"}
        )]
    else
        doc(
            $config:data-root || "/demo/Kirchner-Michaelis-1907.xml"
        )//tei:entry[ft:query(., "lemma:*", map {"leading-wildcard": "yes", "filter-rewrite": "yes", "fields": "lemma"})]
    let $byLetter := map:merge(
        for $entry in $entries
        let $term := upper-case(ft:field($entry, "lemma"))
        order by $term
        group by $letter := substring($term, 1, 1)
        return map:entry($letter, $entry)
    )
    let $letter := if ((count($entries) < $limit)) then
        "[A-Z]"
    else if (empty($letterParam) or $letterParam = "") then
        head(sort(map:keys($byLetter)))
    else
        $letterParam
    let $itemsToShow := if ($letter = "[A-Z]") then
        $entries
    else
        $byLetter($letter)
    return map {
    ...
    array
        {
            ...
            return <div class="term">
                <pb-link emit="detail" params='{{"search": "{ $lemmaField }"}}' subscribe="detail">
                    { $lemmaField }
                </pb-link>
            </div>
        }
