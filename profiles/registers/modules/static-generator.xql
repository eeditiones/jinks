    ...
    array
        {
            ...
            return (static:generate-from-config($context), sg:people($context), sg:places($context))
        }
};
...
declare %private function sg:people ($context as map(*)) {
    ...
    return (
        let $grouped :=
            ...
            return map {"page": $letter, "data": array { $person }}
        return static:split(
            $context,
            $grouped,
            "static/templates/people.html",
            function ($context as map(*), $page) { "people/" || encode-for-uri($page) }
        ),
        for $person in $people?*
        ...
        return static:paginate(
            ...
            function ($context as map(*), $n as xs:int) { "people/" || $person?id }
        ),
...
declare %private function sg:places ($context as map(*)) {
    ...
    return (
        let $grouped :=
            ...
            order by $letter collation "?lang=pl"
            return map {"page": $letter, "data": array { $place }}
        return static:split(
            map:merge(($context, map:entry("geodata", $places))),
            $grouped,
            "static/templates/places.html",
            function ($context as map(*), $page) { "places/" || encode-for-uri($page) }
        ),
        for $place in $places?*
        ...
        return static:paginate(
            ...
            function ($context as map(*), $n as xs:int) { "places/" || $place?id }
        ),
