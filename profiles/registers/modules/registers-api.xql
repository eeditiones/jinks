...
declare function rview:sort ($people as array(*)*, $dir as xs:string) {
    let $sorted := sort($people, "?lang=de-DE", function ($entry) { $entry?1 })
    return if ($dir = "asc") then
        $sorted
    else
        reverse($sorted)
};
...
declare function rview:people-all ($request as map(*)) {
    let $people := collection($config:register-root)/id(
        $config:register-map?person?id
    )//tei:person[ft:query(., "*", map {"leading-wildcard": "yes", "filter-rewrite": "yes"})]
    let $byKey := for-each(
        $people,
        function ($person as element()) {
            let $label := ft:field($person, "sort-name")
            return [lower-case($label), $person]
        }
    )
...
declare function rview:people-categories ($request as map(*)) {
    ...
    let $people := if ($search and $search != "") then
        collection($config:register-root)/id($config:register-map?person?id)//tei:person[ft:query(
            .,
            "name:(" || $search || "*)"
        )]
    else
        ...
        )/id($config:register-map?person?id)//tei:person[ft:query(., "*", map {"leading-wildcard": "yes", "filter-rewrite": "yes"})]
    let $byKey := for-each(
        $people,
        function ($person as element()) {
            let $label := ft:field($person, "sort-name")
            return [lower-case($label), $label, $person]
        }
    )
    let $sorted := rview:sort($byKey, $sortDir)
    let $letter := if ($limit < 0 or count($people) < $limit) then
        "all"
    else if ($letterParam = "") then
        substring($sorted[1]?1, 1, 1) => upper-case()
    else
        $letterParam
    let $byLetter := if ($letter = "all") then
        $sorted
    else
        filter($sorted, function ($entry) { starts-with($entry?1, lower-case($letter)) })
    return map {
                ...
                array
                    {
                        ...
                        let $hits := count(
                            filter(
                                $sorted,
                                function ($entry) { starts-with($entry?1, lower-case($alpha)) }
                            )
                        )
                        where $hits > 0
                        return map {"category": $alpha, "count": $hits},
                        map {"category": "all", "count": count($sorted)}
                    }
    }
};

declare function rview:output-person-all (
    $list as array(*)*,
    $letter as xs:string,
    $search as xs:string?,
    $odd as xs:string,
    $show-notes as xs:boolean
) {
    array
        {
            for $person in $list
            let $letterParam := if ($letter = "all") then
                substring($person?3/@n, 1, 1)
            else
                $letter
            let $note := $pm-config:web-transform(
                $person?3,
                map {"mode": "register-overview", "show-notes": $show-notes},
                $odd
            )
            return <div class="split-list-item">{ $note }</div>
        }
};
...
declare function rview:detail-html ($request as map(*)) {
    ...
    let $mentions := if ($entry instance of element(tei:person)) then
    ...
    return vapi:html($request, $extConfig)
};
...
declare function rview:places ($request as map(*)) {
    ...
    let $odd := head(($request?parameters?odd, $config:default-odd))
    let $show-notes := $request?parameters?description = "on"
    let $places := if ($search and $search != "") then
        collection($config:register-root)/id($config:register-map?place?id)//tei:place[ft:query(
            .,
            "name:(" || $search || "*)"
        )]
    else
    ...
    let $sorted := sort(
        $places,
        "?lang=de-DE",
        function ($place) { lower-case(($place/tei:placeName)[1]) }
    )
    let $letter := if (count($places) < $limit) then
        "all"
    else if ($letterParam = "") then
                ...
                array
                    {
                        ...
                        let $hits := count(
                            filter(
                                ...
                                function ($entry) {
                                    starts-with(
                                        lower-case(($entry/tei:placeName)[1]),
                                        lower-case($alpha)
                                    )
                                }
                            )
...
declare function rview:output-place (
    $list,
    ...
    array
        {
            ...
            return <div class="place split-list-item">{ $note }</div>
        }
};
...
declare function rview:geonames-link ($id) {
    ...
    return if ($geo) then
        <a
            href="https://www.geonames.org/{ $geo }"
            target="_blank"
        >
                w geonames
                <iron-icon icon="maps:place" /></a>
    else (
    )
};
...
declare function rview:bibliography-all ($request as map(*)) {
    ...
    let $byKey := for-each(
        $entries,
        function ($entry as element()) {
            let $label := normalize-space($entry)
            return [lower-case($label), $entry]
        }
    )
    let $sorted := rview:sort($byKey, "asc")
    return array
        {
            for $entry in $sorted
            where $entry?1
            return map {"id": $entry?2/@xml:id/string(), "name": normalize-space($entry?2)}
        }
};
...
declare function rview:bibliography-categories ($request as map(*)) {
    ...
    let $limit := head(($request?parameters?limit, -1))
    let $odd := head(($request?parameters?odd, $config:default-odd))
    let $entries := if ($search and $search != "") then
        collection($config:register-root)/id(
            $config:register-map?bibliography?id
        )//tei:bibl[ft:query(., "name:(" || $search || "*)")]
    else
    ...
    let $byKey := for-each(
        ...
        function ($entry as element()) {
            let $label := normalize-space($entry)
            return [lower-case($label), $label, $entry]
        }
    )
    let $sorted := rview:sort($byKey, $sortDir)
    let $letter := if ($limit < 0 or count($entries) < $limit) then
        "all"
    else if ($letterParam = "") then
        substring($sorted[1]?1, 1, 1) => upper-case()
    else
...
declare function rview:output-bibliography-all (
    $list as array(*)*,
    $letter as xs:string,
    $search as xs:string?,
    $odd as xs:string
    ...
    array
        {
            for $entry in $list
            let $letterParam := if ($letter = "all") then
                substring($entry?3/@n, 1, 1)
            else
                $letter
            let $note := $pm-config:web-transform($entry?3, map {"mode": "register-overview"}, $odd)
            return <div class="split-list-item">{ $note }</div>
        }
};
