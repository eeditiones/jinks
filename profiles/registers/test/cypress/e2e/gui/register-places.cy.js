...
describe("TEI-Publisher Places Register", () => {
    ...
    describe("Places Register Page", () => {
        ...
        it("displays map component", () => {
            ...
            cy.get('pb-leaflet-map, #map, [class*="map"]').should("exist").should("be.visible");
        });
    });
    ...
    describe("Places Register List", () => {
        ...
        it("shows register items when available", () => {
            ...
            cy.get("body").then(($body) => {
                const hasItems =
                    $body.find(
                        'span.place, .place, pb-split-list li, [class*="place"], [class*="register-item"]',
                    ).length > 0 || $body.find("pb-split-list").length > 0;
        ...
        it("displays place names from demo data", () => {
            ...
            cy.get("pb-split-list, .places", { timeout: 10000 }).should("exist");
    ...
    describe("Places Register Navigation", () => {
        it("navigates to place detail page on click", () => {
            ...
            cy.get("body").then(($body) => {
                const placeLinks = $body.find(
                    'a[href*="/places/"], a[href*="places?ref="], span.place a, [class*="place"] a',
                );

                if (placeLinks.length > 0) {
                    // Click first place link
                    cy.wrap(placeLinks.first()).click({ force: true });
        ...
        it("shows geolocation on place detail page", () => {
            ...
            cy.get("body").then(($body) => {
                ...
                if (placeLinks.length > 0) {
                    ...
                    cy.get('pb-geolocation, #locations, [class*="location"]', {
                        timeout: 10000,
                    }).should("exist");
                } else {
