...
describe("TEI-Publisher People Register", () => {
    ...
    describe("People Register List", () => {
        ...
        it("shows register items when available", () => {
            ...
            cy.get("body").then(($body) => {
                const hasItems =
                    $body.find(
                        'span.register-item, .register-item, pb-split-list li, [class*="register-item"]',
        ...
        it("displays register items with names", () => {
            ...
            cy.get("pb-split-list, .people", { timeout: 10000 }).should("exist");
    ...
    describe("People Register Navigation", () => {
        it("navigates to person detail page on click", () => {
            ...
            cy.get("body").then(($body) => {
                const personLinks = $body.find(
                    'a[href*="/people/"], a[href*="people?ref="], span.register-item a',
                );

                if (personLinks.length > 0) {
                    // Click first person link
                    cy.wrap(personLinks.first()).click({ force: true });

                    // Verify URL changed to person detail
                    cy.url({ timeout: 10000 }).should("match", /\/people\/.+/);
