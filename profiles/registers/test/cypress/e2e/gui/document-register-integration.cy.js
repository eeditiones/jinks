...
describe("TEI-Publisher Document-Register Integration", () => {
    ...
    const testDocumentPath =
        Cypress.env("REGISTER_TEST_DOCUMENT") || "test-register/test-document.xml";
    ...
    describe("Register Sidebar Display", () => {
        it("displays register sidebar when enabled", () => {
            ...
            cy.get("body").then(($body) => {
                const hasRegisterSidebar =
                    $body.find(
                        'pb-view[src$="document1"][mode="register"], aside[class*="register"], [class*="sidebar"]',
                    ).length > 0 ||
    ...
    describe("Entity Display in Sidebar", () => {
        it("displays entities when document contains references", () => {
            ...
            cy.get("body").then(($body) => {
                const hasEntities =
                    $body.find(
                        '.register-item, [class*="person"], [class*="place"], pb-view[src$="document1"]',
                    ).length > 0 ||
                ...
                if (hasEntities) {
                    ...
                    cy.log(
                        "No entities found in register sidebar (may be normal if document has no references)",
                    );
                }
            });
        });
        ...
        it("displays place map when places are present", () => {
            ...
            cy.get("body").then(($body) => {
                ...
                if (hasMap) {
                    cy.get("pb-leaflet-map, #map", { timeout: 10000 }).should("exist");
                } else {
    ...
    describe("Entity Navigation from Sidebar", () => {
        it("navigates to entity detail page when entity link is clicked", () => {
            ...
            cy.get("body").then(($body) => {
                ...
                const entityLinks = $body.find(
                    'aside a[href*="/people/"], aside a[href*="/places/"], .register-item a, [class*="register"] a',
                );

                if (entityLinks.length > 0) {
                    // Click first entity link
                    cy.wrap(entityLinks.first()).click({ force: true });

                    // Verify navigation occurred (URL should change to entity detail)
                    cy.url({ timeout: 10000 }).should("match", /\/people\/|\/places\//);
        ...
        it("highlights entities in document when sidebar entry is clicked", () => {
            ...
            cy.get('pb-view[src$="document1"], aside', { timeout: 10000 }).should("exist");
                    ...
                    if (entityLinks.length > 0) {
                        ...
                        cy.url().then((newUrl) => {
                            if (newUrl.includes("/people/") || newUrl.includes("/places/")) {
                                cy.log(
                                    "Navigated to entity detail page - highlighting test passed",
                                );
                            } else if (
                                newUrl.includes("test-document.xml") ||
                                newUrl.includes("test-register/")
                            ) {
    ...
    describe("Register Feature Configuration", () => {
        it("respects register feature enable/disable", () => {
            ...
            cy.get("body").then(($body) => {
                ...
                const hasRegister =
                    $body.find('pb-view[mode="register"], pb-leaflet-map').length > 0;
