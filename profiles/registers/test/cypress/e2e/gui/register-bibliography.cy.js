...
describe("TEI-Publisher Bibliography Register", () => {
    ...
    describe("Bibliography Register List", () => {
        ...
        it("shows bibliography entries when available", () => {
            ...
            cy.get("body").then(($body) => {
                const hasEntries =
                    $body.find(
                        'table tr, tbody tr, .split-list-item, pb-split-list li, [class*="bibliography"]',
                    ).length > 0 || $body.find("pb-split-list, table").length > 0;
    ...
    describe("Bibliography Register Navigation", () => {
        it("navigates to bibliography entry detail page on click", () => {
            ...
            cy.get("body").then(($body) => {
                const entryLinks = $body.find(
                    'a[href*="/bibliography/"], a[href*="bibliography?ref="], table tr a, .split-list-item a, [class*="bibliography"] a',
                );

                if (entryLinks.length > 0) {
                    // Click first entry link
                    cy.wrap(entryLinks.first()).click({ force: true });

                    // Verify URL changed to entry detail
                    cy.url({ timeout: 10000 }).should("match", /\/bibliography\/.+/);
