...
(:~
 ...
 :)
declare %generator:after-write function teip:after-write (
    $context as map(*),
    $target as xs:string
) {
    let $palette := concat(
        "resources/css/palette-",
        ($context?theme?colors?palette, "neutral")[1],
        ".css"
    )
    let $component-styles := if ($context?theme?components?styles) then
        $context?theme?components?styles
    else (
    )
    let $context-with-target := map:merge(($context, map:entry("source", $target)))

    let $theme-bundle := if ($context?production) then (
        cpy:concat(
            $context-with-target,
            (
                $palette,
    ...
    let $components-bundle := cpy:concat(
        $context-with-target,
        (
            $palette,
