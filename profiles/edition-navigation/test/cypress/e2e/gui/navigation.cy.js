...
describe("TEI-Publisher Edition Navigation", () => {
    ...
    describe("Navigation Component", () => {
        it("displays navigation components when available", () => {
            ...
            cy.get("body").then(($body) => {
                const hasNavigation =
                    $body.find(
                        'pb-navigation, [class*="navigation"], button[aria-label*="previous"], button[aria-label*="next"]',
                    ).length > 0 || $body.find("pb-view").length > 0;
    ...
    describe("Navigation via Component", () => {
        it("navigates forward when next button is clicked", () => {
            ...
            cy.get("body").then(($body) => {
                const nextButtons = $body.find(
                    'pb-navigation[keyboard=right], button[aria-label*="next"], a[title*="next"], iron-icon[icon="chevron-right"]',
                );
                ...
                if (nextButtons.length > 0) {
                    ...
                    cy.log(
                        "No explicit navigation buttons found, using keyboard navigation instead",
                    );
                }
            });
        });
        ...
        it("navigates backward when previous button is clicked", () => {
            ...
            cy.get("body").then(($body) => {
                const prevButtons = $body.find(
                    'pb-navigation[keyboard=left], button[aria-label*="previous"], a[title*="previous"], iron-icon[icon="chevron-left"]',
                );
