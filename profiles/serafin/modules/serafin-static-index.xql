...
declare function ridx:entry ($request as map(*)) {
    ...
    let $root := if (ends-with($config:data-default, "/")) then
        $config:data-default
    else
        $config:data-default || "/"
    return [
        map {
            "content":
                nlp:extract-plain-text($doc//tei:text[@type = "source"], true()) => string-join(),
            "translation":
                nlp:extract-plain-text($doc//tei:text[@type = "translation"], true())
                    => string-join(),
            "commentary":
                nlp:extract-plain-text($doc//tei:text[@type = "source"]//tei:note, false())
                    => string-join(),
            "title":
                nlp:extract-plain-text($doc//tei:titleStmt/tei:title[not(@level)], false())
                    => string-join(),
            "link":
                $pathPrefix ||
                    "/" ||
                    substring-after(document-uri(root($doc)), $root) ||
                    "/1/index.html",
    ...
    array
        {
            ...
            return map {"id": $id, "place": $place/tei:placeName[@type = "main"]/string()}
        }
};
