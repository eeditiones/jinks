...
declare variable $idx:app-root := let $rawPath := system:get-module-load-path()
return (: strip the xmldb: part :) if (starts-with($rawPath, "xmldb:exist://")) then
...
declare function idx:get-metadata ($root as element(), $field as xs:string) {
    ...
    return switch ($field)
        ...
        case "content" return
            (root($root)//body, $root/dbk:section)
        default return
            ()
};
...
declare function idx:get-genre ($header as element()?) {
    ...
    return $category/ancestor-or-self::tei:category[parent::tei:category]/tei:catDesc
};
...
declare function idx:get-date ($date) {
    if ($date/@when) then
        xs:date(idx:normalize-date($date/@when/string()))
    else if ($date/@notBefore) then
        xs:date(idx:normalize-date($date/@notBefore/string()))
    else if ($date/@notAfter) then
        xs:date(idx:normalize-date($date/@notAfter/string()))
    else (
    )
};
