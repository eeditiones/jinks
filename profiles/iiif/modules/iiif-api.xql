...
declare %private function iiif:image-info ($path as xs:string) {
    let $request := <http:request
        href="{ $iiifc:IMAGE_API_BASE }/{ $path }/info.json"
        method="GET" />
            ...
            [
                map {
                    ...
                    "resource":
                        map {
                            "@id":
                                $iiifc:IMAGE_API_BASE || "/" || $id || "/full/full/0/default.jpg",
                            "@type": "dctypes:Image",
                            "format": "image/jpeg",
                            "width": $info?width,
                            "height": $info?height,
        ...
        "rendering":
            [
                map {
                    "@id":
                        iiif:link(
                            "api/parts/" || encode-for-uri(config:get-relpath($doc)) || "/html"
                        ) ||
                            "?root=" ||
                            util:node-id($pb),
                    "format": "text/html",
...
declare %private function iiif:link ($relpath as xs:string) {
    ...
    let $port := if (request:get-server-port() = (80, 443)) then (
    ) else
        ":" || request:get-server-port()
    return string-join(($host, $port, replace($config:context-path || "/" || $relpath, "//", "/")))
};
    ...
    return map:merge(
        (
            map {
                ...
                "@type": "sc:Manifest",
                "sequences": [map {"@type": "sc:Sequence", "canvases": array { $canvases }}]
            },
