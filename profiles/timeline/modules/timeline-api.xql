...
declare function timeline:timeline ($request as map(*)) {
    ...
    let $datedEntries := filter(
        $entries,
        function ($entry) {
            try {
                let $date := ft:field($entry, "date", "xs:date")
                return exists($date) and year-from-date($date) != 1000
            } catch * { false() }
        }
    )
    return map:merge(
...
declare function timeline:correspondence-entries ($request as map(*)) {
    ...
    return map:merge(
        for $entry in $docs
        let $date := timeline:get-date(
            $entry//tei:correspDesc/tei:correspAction[@type = "sent"]/tei:date
        )
        where exists($date)
        group by $date
        return map:entry(
                    ...
                    <ul>
                        {
                            for $doc in $entry
                            return <li>
                                <a href="{ util:document-name($doc) }">
                                    { $doc//tei:titleStmt/tei:title/string() }
                                </a>
                            </li>
                        }
                    </ul>
...
declare function timeline:get-date ($date) {
    if ($date/@when) then
        xs:date(timeline:normalize-date($date/@when/string()))
    else if ($date/@notBefore) then
        xs:date(timeline:normalize-date($date/@notBefore/string()))
    else if ($date/@notAfter) then
        xs:date(timeline:normalize-date($date/@notAfter/string()))
    else (
    )
};
...
declare function timeline:in-correspondence ($currentDoc as document-node()?, $type as xs:string) {
    if (empty($currentDoc)) then (
        ...
        let $relPath := substring-after(
            util:collection-name($currentDoc),
            $config:data-root || "/"
        ) ||
            "/" ||
            $next
        let $nextDoc := config:get-document($relPath)
        return ($nextDoc, timeline:in-correspondence($nextDoc, $type))
};
