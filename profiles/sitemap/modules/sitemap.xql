...
declare function sitemap:get-sitemap ($request as map(*)) {
    ...
    return if (doc-available($sitemapPath)) then
        router:response(200, "application/xml", doc($sitemapPath))
    else
        router:response(
            404,
            "text/plain",
            "Sitemap not found. Please generate it first using POST /api/actions/sitemap"
        )
};
...
declare function sitemap:generate-sitemap ($request as map(*)) {
    let $appBase := request:get-scheme() ||
        "://" ||
    ...
    return [map {"type": "actions:sitemap", "message": "Sitemap generated in " || $stored}]
};
...
(:~
 ...
 :)
declare %private function sitemap:generate (
    $appBase as xs:string,
    $config as map(*)
) as document-node() {
    let $baseUri := if ($config?features?sitemap?base-uri) then
...
(:~
 ...
 :)
declare %private function sitemap:crawl (
    $appBase as xs:string,
    ...
    let $urls :=
        ...
        return (: Add all pages for this document :) sitemap:get-document-pages(
            $appBase,
            $baseUri,
            $doc
        )
    return distinct-values($urls)
};
...
declare %private function sitemap:get-documents ($appBase as xs:string) as array(*) {
    let $request := <http:request href="{ $appBase }/api/documents" method="GET" />
...
(:~
 ...
 :)
declare %private function sitemap:get-document-pages (
    $appBase as xs:string,
    $baseUri as xs:string,
    $doc as map(*)
) as xs:string* {
...
declare %private function sitemap:get-document-pages-recursive (
    ...
    return if ($response[1]/@status = 200) then
        ...
        let $currentUrl := if (map:contains($json, "id") and $json?id != "") then
            ...
            $baseUri || "/" || $path
        let $nextId := if (map:contains($json, "nextId") and $json?nextId != "") then
            $json?nextId
        else (
        )
        let $next := if (map:contains($json, "next") and $json?next != "") then
            $json?next
        else (
        )
        return (
    ...
    else (: If the request fails, just return the base path :) (
        $baseUri || "/" || $path
    )
};
