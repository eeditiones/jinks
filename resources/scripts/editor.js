window.addEventListener("DOMContentLoaded", () => {
        ...
        try {
            ...
            apps.forEach(async (app) => {
                ...
                if (app.type === "invalid-config") {
                    ...
                    const reason =
                        app.invalidReason === "dependency-error"
                            ? "dependency-error"
                            : "invalid-json";
    ...
    function loadApp(app) {
        ...
        document
            .getElementById("fileManager")
            .setAttribute("root", `/db/apps/${appConfig.pkg.abbrev}`);
        ...
        form.querySelectorAll('[name="feature"],[name="blueprint"],[name="theme"]').forEach(
            (input) => {
                input.checked = appConfig.extends.includes(input.value);
            },
        );
    ...
    async function doDeploy(config) {
        return displaySpinnerDuringCallback(`Deploying app ${config.pkg.abbrev}…`, async () => {
            ...
            try {
                ...
                if (
                    config.profile &&
                    ["blueprint", "base", "theme", "feature"].includes(config.profile.type)
                ) {
    ...
    async function process(dryRun) {
        await blockUiDuringCallback(async () => {
            const result = await displaySpinnerDuringCallback(
                ...
                async () => {
                    try {
                        ...
                        document
                            .querySelector("#output-dialog")
                            .querySelector(".apply-config").style.display = "none";
                ...
                if (
                    ...
                    result.messages.forEach((message) => {
                        ...
                        if (message.type === "conflict" && message.incoming) {
                            document
                                .querySelector("#output-dialog")
                                .querySelector(".apply-config").style.display = "flex";
                            ...
                            cmpButton.addEventListener("click", (ev) => {
                                ...
                                li.appendChild(diff);
                                const url = new URL(
                                    `api/source?path=${encodeURIComponent(message.source)}`,
                                    window.location,
                                );
                                fetch(url)
                                    .then((response) => {
            ...
            if (result.nextStep.action === "DEPLOY" || result.config._update === false) {
    ...
    async function executeAction(pkgAbbrev, actionName, params) {
        ...
        await blockUiDuringCallback(async () => {
            const result = await displaySpinnerDuringCallback("Running action", async () => {
    ...
    function updateConfig(updateEditor = true) {
        getConfig(appConfig).then(async (config) => {
            ...
            const bootstrapCheckbox = form.querySelector(
                'input[type="checkbox"][name="bootstrap"]:checked',
            );
            ...
            if (bootstrapCheckbox) {
                ...
                Object.keys(config.theme?.colors?.palettes || {}).forEach((palette) => {
                    const colorPaletteInput = form.querySelector(
                        `.color-scheme-option[data-palette="${palette}"]`,
                    );
    ...
    function update(updateEditor = true) {
        ...
        if (formData.get("abbrev") !== "") {
            ...
            if (formData.get("id") === "") {
                form.querySelector('[name="id"]').value =
                    `https://e-editiones.org/apps/${formData.get("abbrev")}`;
            }
        }
        ...
        if (formData.get("bootstrap")) {
            ...
            appConfig.extends = [formData.get("bootstrap")];
            appConfig.profile = {
    ...
    function filterProfilesByBaseProfile() {
        ...
        const profileCheckboxes = form.querySelectorAll(
            'input[type="checkbox"][name="feature"], input[type="checkbox"][name="blueprint"], input[type="checkbox"][name="theme"]',
        );
    ...
    function handleApplyConfig() {
        ...
        if (overwrite === "reinstall") {
            ...
            messageDialog
                .confirm(
                    "Warning",
                    `This will completely reinstall the application. Local changes will be lost.`,
                )
                .then(
                    () => {
                        process(false);
    ...
    form.querySelectorAll('input[type="text"]:not(.action),textarea[name="description"]').forEach(
    ...
    form.querySelectorAll('input[type="checkbox"][name="feature"]').forEach((control) =>
        control.addEventListener("change", toggleFeature),
    );
    form.querySelectorAll('input[type="checkbox"][name="theme"]').forEach((control) =>
        control.addEventListener("change", toggleFeature),
    );
    form.querySelectorAll('input[type="checkbox"][name="blueprint"]').forEach((control) =>
        control.addEventListener("change", toggleFeature),
    );
    form.querySelectorAll('input[type="checkbox"][name="bootstrap"]').forEach((control) =>
        control.addEventListener("change", toggleFeature),
    );
    ...
    function initializeColorPicker() {
        ...
        paletteContainer.querySelector(`input[name="color-palette"][value="${palette}"]`).checked =
            true;
    ...
    function updateColorPaletteSelection(config = appConfig) {
        ...
        const colorPaletteInput = form.querySelector(
            `input[name="color-palette"][value="${palette}"]`,
        );
        if (colorPaletteInput) {
