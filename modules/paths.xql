...
declare %private function path:mkcol-recursive (
    $collection,
    $components,
    ...
    if (exists($components)) then
        let $permissions := if ($permissions) then
...
declare %private function path:mkcol ($path, $userData as xs:string*, $permissions as xs:string?) {
    let $path := if (starts-with($path, "/db/")) then
        ...
        $path
    return path:mkcol-recursive("/db", tokenize($path, "/"), $userData, $permissions)
};
...
declare function path:rmcol ($context as map(*), $path as xs:string) {
    let $absPath := path:resolve-path($context?target, $path)
    return xmldb:remove($absPath)
};

declare function path:get-package-target ($uri as xs:string?) {
    if (not(repo:list()[. = $uri])) then (
    ) else
        let $repoXML := repo:get-resource($uri, "repo.xml")
            => util:binary-to-string()
            => parse-xml()
        return if ($repoXML//repo:target) then
