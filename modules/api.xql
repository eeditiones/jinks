...
declare function api:configurations ($request as map(*)) {
    let $installed :=
        ...
        return if (util:binary-doc-available($configPath)) then
            ...
            return if (map:contains($config, "type")) then
                map {
                    "type": "profile",
                    "profile": $collection,
...
declare function api:profiles () {
    ...
    order by if (map:contains($config, "order")) then
            number($config?order)
        else
            100
    return map:merge(($config, map {"path": $collection})),
    for $collection in xmldb:get-child-collections(repo:get-root())
    let $config := generator:load-json(
        repo:get-root() || "/" || $collection || "/config.json",
        map {}
    )
