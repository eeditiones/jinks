...
declare function api:resolver ($relPath as xs:string) as map(*)? {
    ...
    let $content := if (util:binary-doc-available($path)) then
    ...
    return if ($content) then
        map {"path": $path, "content": $content}
    else (
    )
};

declare function api:generator ($request as map(*)) {
    let $config := if ($request?body instance of array(*)) then
        $request?body?1
    else
        $request?body
    ...
    return generator:process(
        map {"overwrite": $overwrite, "dry": $dryRun, "last-modified": $lastModified},
        $config?config
    )
};

declare function api:expand-template ($request as map(*)) {
    let $template := if ($request?body instance of map(*)) then
        $request?body?template
    else
        $request?body
    let $params := if ($request?body instance of map(*)) then
        head(($request?body?params, map {}))
    else
        map {}
    let $mode := if ($request?body instance of map(*)) then
        $request?body?mode
    else (
    )
    return try {
        if ($request?parameters?force-error = true()) then
            ...
            tmpl:process(
                ...
                map {
                    ...
                    "modules":
                        map {
                            "https://tei-publisher.com/generator/xquery/config":
                                map {"prefix": "config", "at": "modules/config.xql"}
                        }
                }
            )
    } catch * {
...
declare function api:configurations ($request as map(*)) {
    let $installed :=
        ...
        return if (util:binary-doc-available($configPath)) then
            let $config := generator:load-json-safe($configPath)
            return if ($config?_jsonError) then
    ...
    return array { $installed, $profiles }
};

declare function api:expand-config ($request as map(*)) {
    let $userConfig := $request?body
    return generator:extends($userConfig)
};

declare function api:profiles () {
    for $collection in xmldb:get-child-collections($config:app-root || "/profiles")
    let $config := generator:load-json-safe(
        $config:app-root || "/profiles/" || $collection || "/config.json"
    )
    order by if ($config?_jsonError) then
            999
    ...
    return if ($config?_jsonError) then
        ...
        map:merge(($config, map {"path": $collection})),
    for $collection in xmldb:get-child-collections(repo:get-root())
...
declare function api:page ($request as map(*)) {
    ...
    return if (exists($doc)) then
        ...
        return roaster:response(200, $mime, $output)
    else
...
declare function api:profile-documentation ($request as map(*)) {
    ...
    let $config := generator:load-json(
        $config:app-root || "/" || $collection || "/config.json",
        map {}
    )
            ...
            map {
                ...
                "templating":
                    map {
                        "modules":
                            map {
                                "http://e-editiones.org/jinks/templates/util":
                                    map {"prefix": "tu", "at": "modules/template-utils.xql"}
                            }
                    }
            }
        ),
...
declare function api:doc ($request as map(*)) {
    ...
    return if (exists($doc)) then
        let $context := map {
            "title": "jinks",
            "templating": map {"extends": "pages/documentation.html"}
        }
        let $output := tmpl:process(
            $doc,
            $context,
        ...
        return roaster:response(200, "text/html", $output)
    else
        error($errors:NOT_FOUND, $path || " not found")
};
...
declare function api:resolve-conflict ($request as map(*)) {
    ...
    let $path := xmldb:decode($request?parameters?path)
    return api:resolve-conflicts($id, $path)
};
...
declare %private function api:resolve-conflicts ($appId as xs:string, $paths as xs:string*) {
    ...
    return if ($target) then
        ...
        let $updated := fold-right(
            $paths,
            $json,
            function ($path, $input) { map:remove($input, $path) }
        )
...
declare %private function api:sub-collections (
    $root as xs:string,
...
declare %private function api:collections (
    $root as xs:string,
    $child as xs:string,
    $user as xs:string
) as map(*)? {
    if (sm:has-access(xs:anyURI($root), "x")) then
        ...
        return if (sm:has-access(xs:anyURI($root), "r")) then
            map:merge(
                (
                    map {
...
declare %private function api:list-collections ($request as map(*)) {
    ...
    return roaster:response(
        200,
        "application/json",
        array { api:collections($root, $collName, $user) }
    )
};

declare %private function api:list-collection-contents (
    $collection as xs:string,
    $user as xs:string,
    $filter as xs:string?
) as xs:string* {
    ...
    let $resources :=
        ...
        return $r
    let $all := if ($filter) then
        ($subcollections, $resources)[contains(., $filter)]
    else (
        $subcollections, $resources
    )
    for $resource in $all
...
declare %private function api:list-resources ($request as map(*)) {
    ...
    let $end := if ($endParam gt $count) then
        $count
    else
        $endParam
    ...
    let $items := (
        ...
        return let $permissions := sm:get-permissions(xs:anyURI($path))/sm:permission
            ...
            let $permDisplay := if ($isCollection) then
                "c"
            else
                "-" ||
                    $permStr ||
                    (
                        if ($permissions/sm:acl/@entries ne "0") then
                            "+"
                        else
                            ""
                    )
...
declare function api:create-collection ($request as map(*)) {
    ...
    let $user := api:get-user()
    return if (sm:has-access(xs:anyURI($parent), "w")) then
        try {
            let $_ := util:log("INFO", "Creating collection " || $parent || "/" || $collName)
            let $_ := xmldb:create-collection($parent, $collName)
            return roaster:response(200, "application/json", map {"status": "ok"})
        } catch * {
            roaster:response(
                500,
                "application/json",
                map {"status": "fail", "message": $err:description}
            )
        }
    else
        roaster:response(
            ...
            map {
                "status": "fail",
                "message":
                    "You are not allowed to write to collection " ||
                        xmldb:decode-uri(xs:anyURI($parent))
            }
        )
};

declare %private function api:delete-collection (
    $collName as xs:string,
    $user as xs:string
) as map(*) {
    if (sm:has-access(xs:anyURI($collName), "w")) then
        ...
        } catch * { map {"status": "fail", "item": $collName, "message": $err:description} }
    else
        map {
            "status": "fail",
            "item": $collName,
            "message":
                "You are not allowed to write to collection " ||
                    xmldb:decode-uri(xs:anyURI($collName))
        }
};

declare %private function api:delete-resource (
    $collection as xs:string,
    ...
    let $canWrite := sm:has-access(xs:anyURI($resource), "w") and
        sm:has-access(xs:anyURI($resource-collection), "w")
    return if ($canWrite) then
        try {
            let $_ := xmldb:remove($resource-collection, $resource-name)
            return map {"status": "ok"}
        } catch * { map {"status": "fail", "item": $resource, "message": $err:description} }
    else
        map {
            "status": "fail",
            "item": $resource,
...
declare function api:delete-resources ($request as map(*)) {
    ...
    let $selections := if ($removeParam instance of array(*)) then
        $removeParam?*
    ...
    let $results :=
        ...
        let $path := if (starts-with($selection, "/")) then
            $selection
        else
            $collection || "/" || $selection
        let $isCollection := xmldb:collection-available($path)
        let $response := if ($isCollection) then
            api:delete-collection($path, $user)
        else
            api:delete-resource($collection, $path, $user)
        return $response
    return if (some $r in $results satisfies $r?status = "fail") then
...
declare %private function api:copy-or-move (
    $operation as xs:string,
    $sourceCollection as xs:string,
    $target as xs:string,
    ...
    if (sm:has-access(xs:anyURI($target), "w")) then
        let $results :=
            ...
            let $sourcePath := if (starts-with($source, "/")) then
                $source
            else
            ...
            return try {
                if ($isCollection) then
                    ...
                    return map {"status": "ok"}
            } catch * { map {"status": "fail", "message": $err:description, "code": $err:code} }
        return if (some $r in $results satisfies $r?status = "fail") then
            let $failures := $results[?status = "fail"]
            return roaster:response(
                500,
        ...
        roaster:response(
            ...
            map {
                ...
                "message":
                    "You are not allowed to write to collection " ||
                        xmldb:decode-uri(xs:anyURI($target))
            }
        )
};
...
declare function api:copy-resources ($request as map(*)) {
    ...
    let $sources := if ($body?sources instance of array(*)) then
        $body?sources?*
    else
        $body?sources
    let $user := api:get-user()
    return api:copy-or-move("copy", $collection, $target, $sources, $user)
};
...
declare function api:move-resources ($request as map(*)) {
    ...
    let $sources := if ($body?sources instance of array(*)) then
        ...
        $body?sources
    let $user := api:get-user()
    return api:copy-or-move("move", $collection, $target, $sources, $user)
};
...
declare function api:rename-resource ($request as map(*)) {
    ...
    return if (not($newName)) then
        roaster:response(
            400,
            "application/json",
            map {"status": "fail", "message": "Missing required parameter: name"}
        )
    else
        try {
            let $isCollection := xmldb:collection-available($collection || "/" || $resource)
            let $_ := if ($isCollection) then
            ...
            roaster:response(
                500,
                "application/json",
                map {"status": "fail", "message": $err:description, "code": $err:code}
            )
        }
};
...
declare %private function api:get-property-map ($resource as xs:string) as map(*) {
    ...
    return if ($isCollection) then
        map {
            "owner": $permissions/@owner/string(),
            "group": $permissions/@group/string(),
            "last-modified":
                format-dateTime(xmldb:created($resource), "[MNn] [D00] [Y0000] [H00]:[m00]:[s00]"),
        ...
        return map {
            ...
            "last-modified":
                format-dateTime(
                    xmldb:last-modified($components[1], $components[2]),
                    "[MNn] [D00] [Y0000] [H00]:[m00]:[s00]"
                ),
            "permissions": string($permissions/@mode),
            "mime": xmldb:get-mime-type(xs:anyURI($resource))
        }
...
declare %private function api:merge-resource-properties ($resources as xs:string*) as map(*) {
    api:merge-properties(
        for $resource in $resources
        return api:get-property-map($resource)
    )
};
...
declare %private function api:get-permissions ($perms as xs:string) as element() {
    <table>
        <tr><th>User</th><th>Group</th><th>Other</th></tr>
        <tr>
            <td>
...
declare function api:get-properties ($request as map(*)) {
    ...
    let $resources := if ($resourcesParam instance of array(*)) then
        $resourcesParam?*
    ...
    let $html := <form action="" id="browsing-dialog-form">
        <fieldset>
            {
                if ($props("mime") != "") then
                    <div class="control-group">
                        <label for="mime">Mime:</label>
                        <input name="mime" type="text" value="{ $props("mime") }" />
                    </div>
                else (
                )
            }
    ...
    return roaster:response(200, "text/html", $html)
};

declare %private function api:permissions-from-body ($body as map(*)) as xs:string? {
    let $perms := $body?permissions
    return if (exists($perms)) then
        let $rwx :=
            for $type in ("u", "g", "o")
            for $perm in ("r", "w", "x")
            let $key := $type || $perm
            let $param := if ($perms instance of map(*)) then
                $perms?($key)
            else
                $body?($key)
            return concat(
                $type,
                if ($param = true() or $param = "true" or $param = 1) then
                    "+"
                else
                    "-",
                $perm
        ...
        let $special := (
            let $us := if ($perms instance of map(*)) then
                $perms?us
            else
                $body?us
            let $gs := if ($perms instance of map(*)) then
                $perms?gs
            else
                $body?gs
            let $ot := if ($perms instance of map(*)) then
                $perms?ot
            else
                $body?ot
            return (
                concat(
                    "u",
                    if ($us = true() or $us = "true" or $us = 1) then
                        "+"
                    else
                        "-",
                    "s"
                ...
                concat(
                    "g",
                    if ($gs = true() or $gs = "true" or $gs = 1) then
                        "+"
                    else
                        "-",
                    "s"
                ...
                concat(
                    "o",
                    if ($ot = true() or $ot = "true" or $ot = 1) then
                        "+"
                    else
                        "-",
                    "t"
        ...
        return string-join($rwx, ",") || "," || string-join($special, ",")
    else (
    )
};
...
declare function api:change-properties ($request as map(*)) {
    let $body := $request?body
    let $resources := if ($body?resources instance of array(*)) then
        $body?resources?*
    else
        $body?resources
    ...
    return try {
        ...
        return (
            ...
            if ($permFromBody) then
                sm:chmod($uri, $permFromBody)
            else (
            ),
            if ($mime) then
...
declare %private function api:mkcol-recursive (
    $collection as xs:string,
    $components as xs:string*
) as xs:string? {
...
declare %private function api:store-file (
    $root as xs:string,
    $path as xs:string,
    $data as item()
) as xs:string {
    if (matches($path, "/[^/]+$")) then
        ...
        return xmldb:store($newCol, $split[2], $data)
    else
...
declare %private function api:get-descriptors ($zipPath as xs:string) as element()? {
    ...
    return if (exists($binary)) then
        ...
        error(
            xs:QName("api:not-found"),
            "Could not deploy uploaded xar package: " || $zipPath || " not found."
        )
};
...
declare %private function api:deploy-xar ($name as xs:string, $deploy as xs:boolean?) {
    if ($deploy and ends-with($name, ".xar")) then
        ...
        return (repo:remove($appName), repo:install-and-deploy-from-db($name))
    else (
    )
};
...
declare function api:upload ($request as map(*)) {
    ...
    return if (empty($name) or empty($data)) then
        roaster:response(400, "application/json", map {"error": "Missing file upload"})
    else
        try {
            ...
            let $size := if (exists($components)) then
            ...
            roaster:response(
                500,
                "application/json",
                map {"name": $name, "error": $err:description}
            )
        }
};
...
let $lookup := function ($name as xs:string) {
    try { function-lookup(xs:QName($name), 1) } catch * { () }
}
