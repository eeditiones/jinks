...
declare %private function cpy:save-hash (
    $context as map(*),
    $relPath as xs:string,
    $hash as xs:string
) {
    let $jsonFile := path:resolve-path($context?target, ".jinks.json")
    let $json := if (util:binary-doc-available($jsonFile)) then
        json-doc($jsonFile)
    else
        map {}
    let $updated := map:merge(($json, map {$relPath: $hash}), map {"duplicates": "use-last"})
        => serialize(map {"method": "json", "indent": true()})
    return xmldb:store($context?target, ".jinks.json", $updated, "application/json")[2]
};

declare %private function cpy:load-hash ($context as map(*), $relPath as xs:string) {
    let $jsonFile := path:resolve-path($context?target, ".jinks.json")
    return if (util:binary-doc-available($jsonFile)) then
...
declare function cpy:resource-as-string ($context as map(*), $relPath as xs:string) as map(*) {
    ...
    return map {"path": $path, "content": $content}
};

declare function cpy:expand-template (
    $source as xs:string,
    ...
    } catch * { error($cpy:ERROR_TEMPLATE, $err:description, map {"source": $source}) }
};
...
declare function cpy:copy-resource (
    $context as map(*),
    $source as xs:string,
    $target as xs:string
) {
    ...
    return cpy:overwrite(
        ...
        $relPath,
        $sourcePath,
        function () { cpy:resource-as-string($context, $sourcePath)?content },
        function () {
            xmldb:copy-resource(
...
declare function cpy:concat ($context as map(*), $sourcePaths as xs:string+, $target as xs:string) {
    ...
    let $content := string-join(
        ...
        return cpy:resource-as-string($context, $path)?content,
        "&#10;&#10;"
    )
    let $targetPath := path:resolve-path($context?target, $target)
    let $_ := util:log(
        "info",
        "Concatenated " || $targetPath || " from " || string-join($sourcePaths, ", ")
    )
    return cpy:overwrite(
        $context,
        $target,
        ...
        function () { $content },
        function () { xmldb:store(path:parent($targetPath), path:basename($targetPath), $content) }
    )
};
...
declare function cpy:copy-collection (
    $context as map(*),
    $source as xs:string,
    $target as xs:string
) {
...
declare function cpy:copy-collection (
    ...
    return (
        ...
        return if (matches($resource, $context?template-suffix)) then
...
(:~
 ...
 :)
declare %private function cpy:overwrite (
    $context as map(*),
    $relPath as xs:string,
    ...
    ) (: overwrite, but do not check or store hash :) else if (
        $context?force-overwrite or
            (some $ignorePath in $context?ignore?* satisfies matches($relPath, $ignorePath))
    ) then
        $callback()[2]
    (: we're updating an already installed app :)
        ...
        let $lastModified := xmldb:last-modified(
            path:parent($absSourcePath),
            path:basename($absSourcePath)
        )
        return (: Check timestamp of .jinks.json first to determine if source was modified since last run :) (: Templated source files should always be updated though :) if (
            ...
            return (: Check if there have been changes to the file since it was installed :) if (
                empty($currentHash) or empty($expectedHash) or $currentHash = $expectedHash
            ) then
                ...
                return (:
                 ...
                 :) if (empty($expectedHash) or $contentHash != $expectedHash) then (
                    map {"type": "update", "path": $relPath, "source": $sourcePath},
                    let $stored := $callback()
                    return cpy:save-hash($context, $relPath, cpy:hash($stored))
                ) else (
                )
            else
                (: conflict detected :)
                map {
                    ...
                    "path": $relPath,
                    "source": $path,
                    "hash": map {"original": $expectedHash, "actual": $currentHash},
                    "mime": $mime,
                    "incoming":
