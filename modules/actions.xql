...
declare function action:file-sync ($request as map(*)) {
    ...
    let $sync := try { file:sync($root, $target, map {"indent": false()}) } catch * {
    ...
    return array
        {
            ...
            return map {"type": local-name($file), "message": $path}
        }
};
...
declare function action:reindex ($request as map(*)) {
    let $root := $request?parameters?root
    return (
        let $_ := xmldb:copy-resource(
            $root,
            "collection.xconf",
            "/db/system/config/" || $root,
            "collection.xconf"
        )
        return map {
            "type": "action:reindex",
        ...
        return map {"type": "action:reindex", "message": $root || " reindexed"}
    )
};
...
declare function action:download-app ($request as map(*)) {
    ...
    return response:stream-binary($xar, "media-type=application/zip", $name || ".xar")
};
...
declare %private function action:scan (
    $root as xs:anyURI,
    ...
    if (sm:has-access($root, "rx")) then
        ...
        return action:scan(xs:anyURI($root || "/" || $child), $func)
    else (
    )
};
