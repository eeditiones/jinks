...
let $modulePath := replace(
    $rawPath,
    "^(?:xmldb:exist://(?:embedded-eXist-server|null)?)?(.+)$",
    "$1"
)
return substring-before($modulePath, "/modules");

declare variable $config:temp_directory := $config:app-root || "/temp";

declare variable $config:context-path := request:get-context-path() ||
