...
declare function generator:profile-path ($name as xs:string) {
    ...
    return if (xmldb:collection-available($internalPath)) then
        $internalPath
    else
        for $collection in xmldb:get-child-collections(repo:get-root())
        where $collection = $name
        return repo:get-root() || "/" || $collection
};
...
declare function generator:process ($settings as map(*)?, $config as map(*)?) {
    ...
    let $result :=
        ...
        return generator:write($adjContext, generator:profile-path($profileName), $config)
    let $postProcessed :=
        for $profileName in $context?profiles?*
        return generator:after-write($context, $result, generator:profile-path($profileName))
    let $nextStep := if (not(repo:list() = $context?id)) then
        map {
            "message":
                "Package is not deployed yet. Deploy it by calling /api/generator/{profile}/deploy",
            "action": "DEPLOY"
        }
    else
        map {"message": "Package is already deployed. No action needed", "action": "NONE"}
    return map {
...
declare %private function generator:filter-conflicts ($messages as map(*)*) {
    if (empty($messages)) then (
        ...
        let $laterMessages := filter(
            $tail,
            function ($msg) { $msg?type = "conflict" and $msg?path = $next?path }
        )
        return if (exists($laterMessages)) then
            generator:filter-conflicts($tail)
        else (
            $next, generator:filter-conflicts($tail)
        )
};
...
declare %private function generator:call-prepare (
    $collection as xs:string,
    $baseConfig as map(*)?
) {
    let $prepFunc := generator:find-callback($collection, "prepare")
    return if (exists($prepFunc)) then
...
(:
 ...
 :)
declare %private function generator:write (
    $context as map(*)?,
    $collection as xs:string,
    $appConfig as map(*)
) {
    let $writeFunc := generator:find-callback($collection, "write")
    return if (exists($writeFunc)) then (
        ($writeFunc?2)($context), generator:save-config($context, $appConfig)
    ) else (
        cpy:copy-collection($context), generator:save-config($context, $appConfig)
    )
};

declare function generator:after-write (
    $context as map(*),
    $result as map(*)*,
    $collection as xs:string
) {
    let $func := generator:find-callback($collection, "after-write")
    return if (exists($func)) then
        let $targetCollection := if ($context?_update) then
            path:get-package-target($context?id)
        else
            $context?target
        let $adjContext := map:merge(
            (
        ...
        fold-right(
            ...
            function ($func, $in) {
                if (exists($in)) then
                    ...
                    let $anno := $desc/annotation[@namespace = $generator:NAMESPACE][replace(
...
declare function generator:profile ($name as xs:string) as map(*) {
    ...
    return generator:load-json($collection || "/config.json", map {}) => generator:extends()
};
...
declare %private function generator:config ($settings as map(*)?, $userConfig as map(*)?) {
    ...
    let $installedPkg := if ($config?id) then
    ...
    let $config := map:merge(
        (
            ...
            map {
                ...
                "_update":
                    exists($installedPkg) and $settings?overwrite != $cpy:OVERWRITE_REINSTALL,
...
declare %private function generator:extends ($config as map(*), $profile as xs:string?) {
    if (exists($config?extends)) then
        let $extendedProfiles := if ($config?extends instance of array(*)) then
            $config?extends?*
        else
            $config?extends
        let $extendedConfig :=
            ...
            return generator:load-json(
                generator:profile-path($extProfile) || "/config.json",
                map {}
            )
                => generator:extends($extProfile)
        return tmpl:merge-deep(
            (
                $extendedConfig,
                map {"profiles": array { $extendedConfig?profiles?*, $profile }},
                map:remove($config, "extends")
            )
        )
    else
        map:merge(($config, map {"profiles": array { $config?profiles?*, $profile }}))
};
...
declare function generator:load-json-safe ($path as xs:string) as map(*) {
    if (util:binary-doc-available($path)) then
        try {
            ...
            return if ($parsed instance of map(*)) then
                $parsed
            else
                map {
                    "_jsonError": true(),
    ...
    if ($context?_dry or $context?type = "bootstrap") then (
        ...
        return (
            xmldb:store(
                $context?target,
                "config.json",
                serialize($storeConfig, map {"method": "json", "indent": true()})
            )[2],
            xmldb:store(
                $context?target,
                "context.json",
                serialize($context, map {"method": "json", "indent": true()})
            )[2]
        )
};
