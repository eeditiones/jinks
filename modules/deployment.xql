...
declare %private function deploy:scan-resources (
    $root as xs:anyURI,
    ...
    if (sm:has-access($root, "rx")) then
        ...
        return deploy:scan-resources(xs:anyURI($root || "/" || $child), $func)
    else (
    )
};
...
declare function deploy:package ($collection as xs:string, $expathConf as element()) {
    ...
    return try { xmldb:store($config:temp_directory, $name, $xar, "application/zip") } catch * {
...
declare function deploy:deploy ($collection as xs:string, $expathConf as element()) {
    ...
    return (
        deploy:undeploy($name), repo:install-and-deploy-from-db($pkg), xmldb:remove($collection)
    )
};

declare function deploy:undeploy ($id as xs:string) {
    if (index-of(repo:list(), $id)) then (
        repo:undeploy($id), repo:remove($id)
    ) else (
    )
};
