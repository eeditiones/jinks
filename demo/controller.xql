...
declare function local:get-tp () {
    let $path := collection(repo:get-root())//expath:package[@name =
        "https://e-editiones.org/apps/tei-publisher"]
    return if ($path) then
...
return (: redirect webapp root to index.xml :) if ($exist:path eq "") then
    ...
    return <dispatch xmlns="http://exist.sourceforge.net/NS/exist">
        {
            if ($tp) then
                if (
                    request:get-uri() = "/exist/" and request:get-header("X-Forwarded-URI") = "/"
                ) then
    ...
    <ignore xmlns="http://exist.sourceforge.net/NS/exist"><cache-control cache="yes" /></ignore>
